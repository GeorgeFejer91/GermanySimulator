...
frame zero. CMU also notes that hand and toe data can be noisy, so fingers and
toes are not used as hard validation joints. The hard map stops at wrists and
ankles.
...
plane, samples 20 playable poses, and appends point zero as exact inspection
point 21. `walk-cycle-21.json` is the machine-readable authority and
