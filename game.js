...
humorRepeat.onclick=readHumorPage;humorForm.addEventListener("input",updateHumorContinue);humorForm.onsubmit=event=>{event.preventDefault();if((humorPageIndex===0&&!humorReadComplete)||humorActionStep<activeHumorPage().buttons.length||!humorFieldsValid()){humorForm.reportValidity();updateHumorContinue();return}if(++humorPageIndex<humorPages.length)renderHumorPage();else finishHumorCertification()};
