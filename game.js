...
const state={started:false,modal:false,dialogue:false,wanted:0,offence:"AKTENLAGE: UNAUFFÄLLIG",wantedCooldown:0,policeContactCooldown:0,runTimer:0,runWarned:false,roadTimer:0,roadWarned:false,jayCooldown:0,grassTimer:0,grassWarned:false,gardenCooldown:0,evasionTimer:0,pettyAuditTimer:16,mission:0,forms:0,pfand:0,germanness:0,wurstBadges:new Set(),lawUnlocked:false,lawCooldown:0,ampelClock:0,ampelWait:0,ampelWaitLight:null,crossingRun:null,wasOnRoad:false,crossRewardAt:-9,quizTimer:9,quizApproach:null,quizBag:[],quizCharacter:null,day:1,minutes:480,stadtbild:0,citizen:false,gameOver:false,lowEnergyRemarked:false,rule:0,ruleTimer:0,lang:"de",voiceOn:true,subtitlesOn:false,region:"berlin",regionCooldown:0};
const HUD_REFRESH_SECONDS=.1;
let police=[],policeVehicles=[],policeHelicopters=[],particles=[],width=innerWidth,height=innerHeight,dpr=1,last=performance.now(),hudRefreshTimer=0;
...
const TRAIN_CAR_OFFSETS=[780,520,260,0,-260,-520,-780],TRAIN_CAR_HALF_LENGTH=112,TRAIN_CAR_HALF_WIDTH=46,TRAIN_MIN_GAP=1820,TRAIN_PLAYER_STOP_GAP=970,TRAIN_PLAYER_LOOKAHEAD=2100,TRAIN_BOUNCE_PAUSE=.82,TRAIN_AUDIO_RADIUS=820,TRAIN_NEARBY_AUDIO_RADIUS=360;
const trainSeeds=[
...
const KIESINGER_PLAQUE_LINES=Object.freeze([
 ...
 "1948 trat er der CDU bei. Von 1949 bis 1958 war er Bundestagsabgeordneter, von 1958 bis 1966 Ministerpräsident von Baden-Württemberg.",
 "Am 1. Dezember 1966 wählte ihn der Bundestag zum Kanzler der Großen Koalition aus CDU/CSU und SPD. Er amtierte bis 1969. Das Denkmal fragt nach dem Umgang mit personellen Kontinuitäten aus der NS-Zeit."
]);
const districtLots=[
...
const innerMonologues=Object.freeze({
 berlin:Object.freeze({
  train:["Natürlich verpasse ich den Zug. Even mein Scheitern hat Verspätung.","I can't even catch einen Zug, der selber zu spät ist.","Der Zug findet Deutschland. I don't even find den Bahnsteig."],
  wrongOffice:["Great. Falsches Amt. Ich kriege nicht einmal meine eigene Zuständigkeit hin.","Another Tür, another Beweis, dass ich administrativ nichts auf die Reihe bekomme.","Maybe bin ich nicht lost. Vielleicht bin ich einfach falsch abgeheftet."],
  bicycle:["Natürlich stehe ich im Radweg. I can't even fail in der richtigen Spur.","Sogar ein Fahrrad has more Richtung im Leben als ich."],
  fax:["Ich werde nie anything amounten, außer vielleicht zu einem sauber abgehefteten Irrtum.","Der Faxapparat has a clearer purpose als ich.","Papier rein, Würde raus. At least der Prozess funktioniert."],
  coffee:["Nicht einmal der Kaffeeautomat wants responsibility für mich.","Great. Selbst die Maschine hat mehr Pause verdient als ich."],
  pfand:["Nicht einmal Leergut. I bring literally nichts Verwertbares mit.","Der Automat erkennt keinen Wert. Fair assessment, honestly."],
  wrongTask:["Natürlich ist das noch nicht mein Vorgang. Nichts ist je mein Vorgang, außer das Scheitern.","I can't even be falsch zuständig at the correct time."],
  lost:["Hier ist niemand zuständig. Wahrscheinlich bin ich selbst für nichts zuständig.","At this rate werde ich nie anything amounten—außer einem offenen Vorgang.","Nicht einmal mein Verirren follows einen genehmigten Prozess."],
  exhaustion:["Natürlich bin ich erschöpft. Andere schaffen Dinge; ich verwalte nur mein Scheitern.","My Energie ist alle. Die Enttäuschung remains fully operational."],
  deadline:["Schon wieder ein Tag weg. I am late sogar für mein eigenes Scheitern.","Ein Behördentag weniger, zero erkennbare Entwicklung."]
 }),
 germany:Object.freeze({
  train:["Natürlich verpasse ich den Zug. Sogar mein Scheitern hat Verspätung.","Ich schaffe es nicht einmal in einen Zug, der selbst zu spät kommt.","Der Zug findet seinen Weg. Ich finde nicht einmal den Bahnsteig."],
  wrongOffice:["Großartig. Falsches Amt. Ich bekomme nicht einmal meine eigene Zuständigkeit hin.","Noch eine Tür, noch ein Beweis, dass ich verwaltungstechnisch nichts auf die Reihe bekomme.","Vielleicht bin ich nicht verirrt. Vielleicht bin ich nur falsch abgeheftet."],
  bicycle:["Natürlich stehe ich im Radweg. Ich kann nicht einmal in der richtigen Spur scheitern.","Sogar ein Fahrrad hat mehr Richtung im Leben als ich."],
  fax:["Ich werde nie etwas zustande bringen, außer vielleicht einen sauber abgehefteten Irrtum.","Der Faxapparat hat einen klareren Zweck als ich.","Papier hinein, Würde heraus. Wenigstens funktioniert der Vorgang."],
  coffee:["Nicht einmal der Kaffeeautomat will für mich zuständig sein.","Großartig. Selbst die Maschine hat mehr Pause verdient als ich."],
  pfand:["Nicht einmal Leergut. Ich bringe wirklich nichts Verwertbares mit.","Der Automat erkennt keinen Wert. Eine nachvollziehbare Einschätzung."],
  wrongTask:["Natürlich ist das noch nicht mein Vorgang. Nichts ist je mein Vorgang, außer das Scheitern.","Ich kann nicht einmal zum richtigen Zeitpunkt falsch zuständig sein."],
  lost:["Hier ist niemand zuständig. Wahrscheinlich bin ich selbst für nichts zuständig.","In diesem Tempo werde ich nie etwas zustande bringen – außer einen offenen Vorgang.","Nicht einmal mein Verirren folgt einem genehmigten Verfahren."],
  exhaustion:["Natürlich bin ich erschöpft. Andere schaffen Dinge; ich verwalte nur mein Scheitern.","Meine Energie ist verbraucht. Die Enttäuschung arbeitet störungsfrei weiter."],
  deadline:["Schon wieder ein Tag verloren. Ich komme sogar für mein eigenes Scheitern zu spät.","Ein Behördentag weniger, weiterhin keine erkennbare Entwicklung."]
 })
});
...
// Civic entries are lightly condensed from BAMF's 07.05.2025 Gesamtfragenkatalog; FS and grammar entries are original game questions. Answers are zero-based choice indices.
const citizenshipQuestions=[
 ...
 {source:"G-B1-01",type:"grammar",level:"B1",question:"Welche Version ist grammatisch richtig?",choices:["Ich warte, weil der Bescheid noch nicht angekommen ist.","Ich warte, weil der Bescheid ist noch nicht angekommen.","Ich warte, weil noch nicht ist der Bescheid angekommen.","Ich warte, denn der Bescheid noch nicht angekommen ist."],answer:0},
 {source:"G-B1-02",type:"grammar",level:"B1",question:"Ergänzen Sie: Ich wohne ___ drei Jahren in Berlin.",choices:["seit","vor","ab","während"],answer:0},
 {source:"G-B1-03",type:"grammar",level:"B1",question:"Welche Bitte ist höflich und korrekt?",choices:["Erklären Sie mir den Antrag, sofort.","Könnten Sie mir bitte den Antrag erklären?","Sie könnten mir den Antrag erklärt.","Ob Sie mir den Antrag erklären würden!"],answer:1},
 {source:"G-B1-04",type:"grammar",level:"B1",question:"Welcher Satz verwendet den Akkusativ korrekt?",choices:["Ich lege der Bescheid auf den Tisch.","Ich lege dem Bescheid auf den Tisch.","Ich lege den Bescheid auf den Tisch.","Ich lege des Bescheids auf den Tisch."],answer:2},
 {source:"G-B1-05",type:"grammar",level:"B1",question:"Welcher Relativsatz ist korrekt?",choices:["Das ist die Sachbearbeiterin, die meinen Antrag prüft.","Das ist die Sachbearbeiterin, der meinen Antrag prüft.","Das ist die Sachbearbeiterin, welche prüft meinen Antrag.","Das ist die Sachbearbeiterin, die mein Antrag prüfen."],answer:0},
 {source:"G-B1-06",type:"grammar",level:"B1",question:"Wie lautet der Satz im Perfekt?",choices:["Ich habe das Amt angeruft.","Ich bin das Amt angerufen.","Ich habe das Amt angerufen.","Ich hatte das Amt anrufen."],answer:2},
 {source:"G-B1-07",type:"grammar",level:"B1",question:"Welche indirekte Frage ist korrekt?",choices:["Können Sie mir sagen, wann beginnt der Termin?","Können Sie mir sagen, wann der Termin beginnt?","Können Sie mir sagen, der Termin wann beginnt?","Können Sie mir sagen, wann beginnt?"],answer:1},
 {source:"G-B1-08",type:"grammar",level:"B1",question:"Ergänzen Sie die Adjektivendung: Ich brauche den ausgefüllt___ Antrag.",choices:["e","en","er","es"],answer:1},
 {source:"G-B1-09",type:"grammar",level:"B1",question:"Welcher Satz mit „obwohl“ ist korrekt?",choices:["Obwohl ich hatte einen Termin, musste ich warten.","Obwohl einen Termin ich hatte, musste ich warten.","Obwohl ich einen Termin hatte, musste ich warten.","Obwohl ich einen Termin hatte, ich musste warten."],answer:2},
 {source:"G-B1-10",type:"grammar",level:"B1",question:"Ergänzen Sie: Bitte kommen Sie mit ___ gültigen Pass.",choices:["ein","einen","einem","eines"],answer:2},
 {source:"G-B2-01",type:"grammar",level:"B2",question:"Welche Zeitfolge ist korrekt?",choices:["Nachdem ich den Antrag abgegeben hatte, erhielt ich eine Nummer.","Nachdem ich den Antrag abgegeben habe, erhielt ich gestern eine Nummer gehabt.","Nachdem ich hatte den Antrag abgegeben, erhielt ich eine Nummer.","Nachdem den Antrag ich abgegeben hatte, erhielt ich eine Nummer."],answer:0},
 {source:"G-B2-02",type:"grammar",level:"B2",question:"Welcher irreale Bedingungssatz in der Vergangenheit ist korrekt?",choices:["Wenn ich früher angerufen hätte, hätte ich einen Termin bekommen.","Wenn ich früher angerufen würde, habe ich einen Termin bekommen.","Wenn ich hätte früher angerufen, würde ich einen Termin bekommen gehabt.","Wenn ich früher anrief, hätte ich einen Termin bekomme."],answer:0},
 {source:"G-B2-03",type:"grammar",level:"B2",question:"Welche Passivform ist korrekt?",choices:["Der Antrag wird von der Behörde geprüft.","Der Antrag ist von der Behörde prüfen.","Der Antrag wird von der Behörde prüfen.","Der Antrag wurde von der Behörde geprüft werden."],answer:0},
 {source:"G-B2-04",type:"grammar",level:"B2",question:"Welcher Relativsatz mit Präposition ist korrekt?",choices:["Das ist das Amt, bei das ich angerufen habe.","Das ist das Amt, bei dem ich angerufen habe.","Das ist das Amt, wo bei ich angerufen habe.","Das ist das Amt, dem ich bei angerufen habe."],answer:1},
 {source:"G-B2-05",type:"grammar",level:"B2",question:"Welche Konstruktion mit „je … desto“ ist korrekt?",choices:["Je länger das Verfahren dauert, desto ungeduldiger werde ich.","Je das Verfahren länger dauert, desto ich ungeduldiger werde.","Desto länger das Verfahren dauert, je ungeduldiger werde ich.","Je länger dauert das Verfahren, desto werde ungeduldiger ich."],answer:0},
 {source:"G-B2-06",type:"grammar",level:"B2",question:"Welche Form der n-Deklination ist korrekt?",choices:["Ich spreche mit dem Beamte.","Ich spreche mit des Beamten.","Ich spreche mit den Beamter.","Ich spreche mit dem Beamten."],answer:3},
 {source:"G-B2-07",type:"grammar",level:"B2",question:"Welche Nominalisierung ist korrekt?",choices:["Aufgrund der verspäteten Zustellung fehlte der Nachweis.","Aufgrund die verspätete Zustellung fehlte der Nachweis.","Aufgrund verspätet zustellen fehlte der Nachweis.","Aufgrund der Zustellung verspätete fehlte der Nachweis."],answer:0},
 {source:"G-B2-08",type:"grammar",level:"B2",question:"Welcher Konnektor passt? Der Antrag war vollständig; ___ wurde er zurückgeschickt.",choices:["deshalb","dennoch","sobald","damit"],answer:1},
 {source:"G-B2-09",type:"grammar",level:"B2",question:"Welcher Satz mit Pronominaladverb ist korrekt?",choices:["Ich freue mich darauf, dass der Bescheid kommt.","Ich freue darauf mich, dass der Bescheid kommt.","Ich freue mich auf, dass der Bescheid kommt.","Ich freue darüber, den Bescheid kommt."],answer:0},
 {source:"G-B2-10",type:"grammar",level:"B2",question:"Welche Stufe gehört nicht zum Gemeinsamen Europäischen Referenzrahmen?",choices:["A2","B2","B3","C1"],answer:2},
 {source:"G-C1-01",type:"grammar",level:"C1",question:"Welche indirekte Rede im Konjunktiv I ist korrekt?",choices:["Sie sagte, der Bescheid sei versandt worden.","Sie sagte, der Bescheid wäre versendet gewesen sein.","Sie sagte, der Bescheid ist versandt worden sei.","Sie sagte, sei der Bescheid versandt worden."],answer:0},
 {source:"G-C1-02",type:"grammar",level:"C1",question:"Welche Passiversatzform ist korrekt?",choices:["Der Antrag lässt online nicht einreichen.","Der Antrag lässt sich online nicht einreichen.","Der Antrag ist sich online nicht einzureichen lassen.","Der Antrag lässt sich online nicht eingereicht."],answer:1},
 {source:"G-C1-03",type:"grammar",level:"C1",question:"Welche nominale Formulierung ist korrekt?",choices:["Nach Prüfung der Unterlagen erfolgte die Genehmigung.","Nach Prüfen die Unterlagen erfolgte die Genehmigung.","Nach der Unterlagen Prüfung erfolgte genehmigen.","Nach geprüfte Unterlagen ist die Genehmigung erfolgen."],answer:0},
 {source:"G-C1-04",type:"grammar",level:"C1",question:"Welche Partizipialgruppe ist korrekt?",choices:["die gestern eingegangenen Unterlagen","die gestern eingehende Unterlagen gewesen","die gestern eingegangen Unterlagen","die gestern eingehen gewordenen Unterlagen"],answer:0},
 {source:"G-C1-05",type:"grammar",level:"C1",question:"Welcher Satz mit „ohne dass“ ist korrekt?",choices:["Er verließ das Amt, ohne dass sein Antrag bearbeitet worden war.","Er verließ das Amt, ohne dass war sein Antrag bearbeitet.","Er verließ das Amt, ohne sein Antrag dass bearbeitet war.","Er verließ das Amt, ohne dass seinen Antrag bearbeitet worden."],answer:0},
 {source:"G-C1-06",type:"grammar",level:"C1",question:"Welcher Satz mit „sofern“ ist korrekt?",choices:["Sofern alle Nachweise vorliegen, kann der Antrag bearbeitet werden.","Sofern liegen alle Nachweise vor, der Antrag kann bearbeitet werden.","Sofern alle Nachweise vorliegen können, wird der Antrag bearbeiten.","Sofern der Antrag, alle Nachweise liegen vor, kann bearbeitet."],answer:0},
 {source:"G-C1-07",type:"grammar",level:"C1",question:"Welche mehrteilige Konjunktion ist korrekt verwendet?",choices:["Nicht nur fehlte der Stempel, aber auch war die Unterschrift blau.","Nicht nur der Stempel fehlte, sondern auch die Unterschrift war unleserlich.","Nicht nur fehlte der Stempel, sowohl die Unterschrift war unleserlich.","Sondern der Stempel fehlte, nicht nur die Unterschrift."],answer:1},
 {source:"G-C1-08",type:"grammar",level:"C1",question:"Welche Formulierung mit Genitivpräposition ist korrekt?",choices:["Angesichts des fehlenden Nachweises wurde das Verfahren ausgesetzt.","Angesichts dem fehlenden Nachweis wurde das Verfahren ausgesetzt.","Angesichts den fehlenden Nachweis wurde das Verfahren ausgesetzt.","Angesichts der fehlender Nachweis wurde das Verfahren ausgesetzt."],answer:0}
];
const berlinCitizenshipQuestions={
 ...
 "G-B1-01":{"question":"Which Version ist grammatically korrekt?","choices":["Ich warte, weil der Bescheid noch nicht angekommen ist.","Ich warte, weil der Bescheid ist noch nicht angekommen.","Ich warte, weil noch nicht ist der Bescheid angekommen.","Ich warte, denn der Bescheid noch nicht angekommen ist."]},
 "G-B1-02":{"question":"Which Präposition completes den Satz: Ich wohne ___ drei Jahren in Berlin?","choices":["seit","vor","ab","während"]},
 "G-B1-03":{"question":"Which Bitte ist höflich and korrekt?","choices":["Erklären Sie mir den Antrag, sofort.","Könnten Sie mir bitte den Antrag erklären?","Sie könnten mir den Antrag erklärt.","Ob Sie mir den Antrag erklären würden!"]},
 "G-B1-04":{"question":"Which Satz uses den Akkusativ korrekt?","choices":["Ich lege der Bescheid auf den Tisch.","Ich lege dem Bescheid auf den Tisch.","Ich lege den Bescheid auf den Tisch.","Ich lege des Bescheids auf den Tisch."]},
 "G-B1-05":{"question":"Which Relativsatz ist korrekt?","choices":["Das ist die Sachbearbeiterin, die meinen Antrag prüft.","Das ist die Sachbearbeiterin, der meinen Antrag prüft.","Das ist die Sachbearbeiterin, welche prüft meinen Antrag.","Das ist die Sachbearbeiterin, die mein Antrag prüfen."]},
 "G-B1-06":{"question":"How lautet der Satz im Perfekt?","choices":["Ich habe das Amt angeruft.","Ich bin das Amt angerufen.","Ich habe das Amt angerufen.","Ich hatte das Amt anrufen."]},
 "G-B1-07":{"question":"Which indirekte Frage ist korrekt?","choices":["Können Sie mir sagen, wann beginnt der Termin?","Können Sie mir sagen, wann der Termin beginnt?","Können Sie mir sagen, der Termin wann beginnt?","Können Sie mir sagen, wann beginnt?"]},
 "G-B1-08":{"question":"Complete die Adjektivendung: Ich brauche den ausgefüllt___ Antrag.","choices":["e","en","er","es"]},
 "G-B1-09":{"question":"Which Satz mit ‘obwohl’ ist korrekt?","choices":["Obwohl ich hatte einen Termin, musste ich warten.","Obwohl einen Termin ich hatte, musste ich warten.","Obwohl ich einen Termin hatte, musste ich warten.","Obwohl ich einen Termin hatte, ich musste warten."]},
 "G-B1-10":{"question":"Complete bitte: Kommen Sie mit ___ gültigen Pass.","choices":["ein","einen","einem","eines"]},
 "G-B2-01":{"question":"Which Zeitfolge ist korrekt?","choices":["Nachdem ich den Antrag abgegeben hatte, erhielt ich eine Nummer.","Nachdem ich den Antrag abgegeben habe, erhielt ich gestern eine Nummer gehabt.","Nachdem ich hatte den Antrag abgegeben, erhielt ich eine Nummer.","Nachdem den Antrag ich abgegeben hatte, erhielt ich eine Nummer."]},
 "G-B2-02":{"question":"Which irreale Bedingung in der Vergangenheit ist korrekt?","choices":["Wenn ich früher angerufen hätte, hätte ich einen Termin bekommen.","Wenn ich früher angerufen würde, habe ich einen Termin bekommen.","Wenn ich hätte früher angerufen, würde ich einen Termin bekommen gehabt.","Wenn ich früher anrief, hätte ich einen Termin bekomme."]},
 "G-B2-03":{"question":"Which Passivform ist korrekt?","choices":["Der Antrag wird von der Behörde geprüft.","Der Antrag ist von der Behörde prüfen.","Der Antrag wird von der Behörde prüfen.","Der Antrag wurde von der Behörde geprüft werden."]},
 "G-B2-04":{"question":"Which Relativsatz mit Präposition ist korrekt?","choices":["Das ist das Amt, bei das ich angerufen habe.","Das ist das Amt, bei dem ich angerufen habe.","Das ist das Amt, wo bei ich angerufen habe.","Das ist das Amt, dem ich bei angerufen habe."]},
 "G-B2-05":{"question":"Which ‘je … desto’ Konstruktion ist korrekt?","choices":["Je länger das Verfahren dauert, desto ungeduldiger werde ich.","Je das Verfahren länger dauert, desto ich ungeduldiger werde.","Desto länger das Verfahren dauert, je ungeduldiger werde ich.","Je länger dauert das Verfahren, desto werde ungeduldiger ich."]},
 "G-B2-06":{"question":"Which Form der n-Deklination ist korrekt?","choices":["Ich spreche mit dem Beamte.","Ich spreche mit des Beamten.","Ich spreche mit den Beamter.","Ich spreche mit dem Beamten."]},
 "G-B2-07":{"question":"Which Nominalisierung ist korrekt?","choices":["Aufgrund der verspäteten Zustellung fehlte der Nachweis.","Aufgrund die verspätete Zustellung fehlte der Nachweis.","Aufgrund verspätet zustellen fehlte der Nachweis.","Aufgrund der Zustellung verspätete fehlte der Nachweis."]},
 "G-B2-08":{"question":"Which Konnektor passt? Der Antrag war vollständig; ___ wurde er zurückgeschickt.","choices":["deshalb","dennoch","sobald","damit"]},
 "G-B2-09":{"question":"Which Satz mit Pronominaladverb ist korrekt?","choices":["Ich freue mich darauf, dass der Bescheid kommt.","Ich freue darauf mich, dass der Bescheid kommt.","Ich freue mich auf, dass der Bescheid kommt.","Ich freue darüber, den Bescheid kommt."]},
 "G-B2-10":{"question":"Which Stufe gehört not zum Gemeinsamen Europäischen Referenzrahmen?","choices":["A2","B2","B3","C1"]},
 "G-C1-01":{"question":"Which indirekte Rede im Konjunktiv I ist korrekt?","choices":["Sie sagte, der Bescheid sei versandt worden.","Sie sagte, der Bescheid wäre versendet gewesen sein.","Sie sagte, der Bescheid ist versandt worden sei.","Sie sagte, sei der Bescheid versandt worden."]},
 "G-C1-02":{"question":"Which Passiversatzform ist korrekt?","choices":["Der Antrag lässt online nicht einreichen.","Der Antrag lässt sich online nicht einreichen.","Der Antrag ist sich online nicht einzureichen lassen.","Der Antrag lässt sich online nicht eingereicht."]},
 "G-C1-03":{"question":"Which nominale Formulierung ist korrekt?","choices":["Nach Prüfung der Unterlagen erfolgte die Genehmigung.","Nach Prüfen die Unterlagen erfolgte die Genehmigung.","Nach der Unterlagen Prüfung erfolgte genehmigen.","Nach geprüfte Unterlagen ist die Genehmigung erfolgen."]},
 "G-C1-04":{"question":"Which Partizipialgruppe ist korrekt?","choices":["die gestern eingegangenen Unterlagen","die gestern eingehende Unterlagen gewesen","die gestern eingegangen Unterlagen","die gestern eingehen gewordenen Unterlagen"]},
 "G-C1-05":{"question":"Which Satz mit ‘ohne dass’ ist korrekt?","choices":["Er verließ das Amt, ohne dass sein Antrag bearbeitet worden war.","Er verließ das Amt, ohne dass war sein Antrag bearbeitet.","Er verließ das Amt, ohne sein Antrag dass bearbeitet war.","Er verließ das Amt, ohne dass seinen Antrag bearbeitet worden."]},
 "G-C1-06":{"question":"Which Satz mit ‘sofern’ ist korrekt?","choices":["Sofern alle Nachweise vorliegen, kann der Antrag bearbeitet werden.","Sofern liegen alle Nachweise vor, der Antrag kann bearbeitet werden.","Sofern alle Nachweise vorliegen können, wird der Antrag bearbeiten.","Sofern der Antrag, alle Nachweise liegen vor, kann bearbeitet."]},
 "G-C1-07":{"question":"Which mehrteilige Konjunktion ist korrekt verwendet?","choices":["Nicht nur fehlte der Stempel, aber auch war die Unterschrift blau.","Nicht nur der Stempel fehlte, sondern auch die Unterschrift war unleserlich.","Nicht nur fehlte der Stempel, sowohl die Unterschrift war unleserlich.","Sondern der Stempel fehlte, nicht nur die Unterschrift."]},
 "G-C1-08":{"question":"Which Formulierung mit Genitivpräposition ist korrekt?","choices":["Angesichts des fehlenden Nachweises wurde das Verfahren ausgesetzt.","Angesichts dem fehlenden Nachweis wurde das Verfahren ausgesetzt.","Angesichts den fehlenden Nachweis wurde das Verfahren ausgesetzt.","Angesichts der fehlender Nachweis wurde das Verfahren ausgesetzt."]}
};
const quizContexts={
 civic:{berlin:"For die Einbürgerung nennt § 10 Absatz 4 StAG grundsätzlich B1; diese Bürgerkunde-Frage is trotzdem keine Sprachprüfung and this is keine Rechtsberatung.",germany:"Für die Einbürgerung nennt § 10 Absatz 4 StAG grundsätzlich B1; diese Bürgerkunde-Frage ist trotzdem keine Sprachprüfung und dies ist keine Rechtsberatung."},
 traffic:{berlin:"For den Führerschein hilft echtes Lernen; diese Igel-und-Laminierung-Frage is ausdrücklich keine echte Prüfungsfrage.",germany:"Für den Führerschein hilft echtes Lernen; diese Igel-und-Laminierung-Frage ist ausdrücklich keine echte Prüfungsfrage."},
 "grammar-b1":{berlin:"You want die deutsche Staatsangehörigkeit? § 10 Absatz 4 StAG nennt grundsätzlich B1. Jetzt prove it mit einer originalen Spielübung.",germany:"Sie möchten die deutsche Staatsangehörigkeit? § 10 Absatz 4 StAG nennt grundsätzlich B1. Beweisen Sie es nun mit einer originalen Spielübung."},
 "grammar-b2":{berlin:"You want Ausbildung oder Beruf? B2 can verlangt werden, aber not pauschal überall. Der konkrete Träger liebt den konkreten Nachweis.",germany:"Sie möchten eine Ausbildung oder einen Beruf ausüben? B2 kann verlangt werden, aber nicht pauschal überall. Der konkrete Träger liebt den konkreten Nachweis."},
 "grammar-c1":{berlin:"You want Universität? Entscheidend is der konkrete Hochschulnachweis; TestDaF TDN 4 in allen Teilen gilt allgemein für uneingeschränkte Zulassung.",germany:"Sie möchten an die Universität? Entscheidend ist der konkrete Hochschulnachweis; TestDaF TDN 4 in allen Teilen gilt allgemein für die uneingeschränkte Zulassung."}
};
const quizCharacterLibrary=window.GermanySimulatorQuizCharacters||{characters:[]},quizCharacterBags=new Map(),quizCharacterFallback={id:"fallback",name:"Herr Becker",title:"BESORGTER BÜRGER",office:"Allgemeine Einmischung",authority:"spontan erklärt",specialty:"alles",categories:["civic","traffic","grammar-b1","grammar-b2","grammar-c1"],portrait:"",alt:"Fiktiver besorgter Bürger"};
function quizQuestionCategory(q){return q.type==="fahrschule"?"traffic":q.type==="grammar"?"grammar-"+q.level.toLowerCase():"civic"}
function nextQuizCharacter(category){
 const candidates=(quizCharacterLibrary.characters||[]).filter(person=>person.categories.includes(category)),pool=candidates.length?candidates:[quizCharacterFallback];let bag=quizCharacterBags.get(category);
 if(!bag?.length){bag=pool.map((_,index)=>index);for(let i=bag.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[bag[i],bag[j]]=[bag[j],bag[i]]}quizCharacterBags.set(category,bag)}
 return pool[bag.pop()]
}
...
function staticBlocked(x,y,r=player.r){if(x<r+9||y<r+9||x>WORLD.w-r-9||y>WORLD.h-r-9)return true;if(dist(x,y,wirtschaftswunderSite.x,wirtschaftswunderSite.y)<r+wirtschaftswunderSite.collisionRadius)return true;if(Math.abs(x-kiesingerMemorial.x)<r+173&&Math.abs(y-kiesingerMemorial.y)<r+147)return true;for(const b of buildings)if(x>b.x-r&&x<b.x+b.w+r&&y>b.y-r&&y<b.y+b.h+r)return true;for(const p of props)if(dist(x,y,p.x,p.y)<r+propRadius(p))return true;for(const o of normObjects)if(dist(x,y,o.x,o.y)<r+26)return true;for(const tree of trees)if(dist(x,y,tree.x,tree.y)<r+TREE_RADIUS)return true;return false}
...
for(const source of [pedestrianBarks.berlin.sidewalk,pedestrianBarks.germany.sidewalk])addSubtitlePool(source.slice(0,6),audioTextLibrary.pools.sidewalk);
for(const source of [pedestrianBarks.berlin.grass,pedestrianBarks.germany.grass])addSubtitlePool(source,audioTextLibrary.pools.grass);
for(const source of [pedestrianBarks.berlin.road,pedestrianBarks.germany.road])addSubtitlePool(source,audioTextLibrary.pools.road);
for(const archetype of crowdArchetypes)for(const region of ["berlin","germany"])addSubtitlePool(archetype.barks[region],audioTextLibrary.pools[archetype.id==="towel-man"?"towelMan":"towelWoman"]);
for(const source of [npcDenglisch,npcLines])addSubtitlePool(source,audioTextLibrary.pools.npc);
if(typeof innerMonologues!=="undefined")for(const region of ["berlin","germany"])for(const [context,source] of Object.entries(innerMonologues[region]))addSubtitlePool(source,audioTextLibrary.pools.innerMonologues?.[context]);
for(const source of [quizApproaches.berlin,quizApproaches.germany])addSubtitlePool(source,audioTextLibrary.pools.quizApproaches);
addSubtitlePool(railLawQuotes,audioTextLibrary.pools.railLaw);addSubtitlePool(lawPowerLines,audioTextLibrary.pools.lawPower);
for(const entry of Object.values(audioTextLibrary.recordings||{}))if(entry.source&&entry.english)subtitleEnglish.set(entry.source,entry.english);
let activeAudioText=null,activeStimulus=null,stimulusTimer=null,stimulusGeneration=0,stimulusSequence=0,nextAmbientAt=0,recordedSpeechSource=null,recordedSpeechGain=null,foregroundBus=null,masterBus=null,subtitleFrame=0;
function dynamicEnglishSubtitle(text){let match=text?.match(/^Für (.+) brauchen Sie Formular (.+)\. Very important\.$/);if(match)return `For ${englishText.get(match[1])||match[1]}, you need form ${match[2]}. Very important.`;match=text?.match(/^Für (.+) benötigen Sie Formular (.+)\.$/);if(match)return `For ${englishText.get(match[1])||match[1]}, you need form ${match[2]}.`;return ""}
function englishSubtitle(item){const recording=item.recording&&audioTextLibrary.recordings?.[item.recording];return item.subtitle||recording?.english||subtitleEnglish.get(item.text)||dynamicEnglishSubtitle(item.text)||item.text||""}
function clearSubtitle(){cancelAnimationFrame(subtitleFrame);subtitleFrame=0;const box=document.getElementById("english-subtitle");box.hidden=true;box.textContent="";document.getElementById("app").classList.remove("subtitles-visible")}
function setSubtitle(text){const box=document.getElementById("english-subtitle");box.textContent=text;box.hidden=!text;document.getElementById("app").classList.toggle("subtitles-visible",!!text)}
function startSubtitle(item,elapsed=0){clearSubtitle();if(!state.subtitlesOn)return;const cues=item.cues||audioTextLibrary.recordings?.[item.recording]?.cues;if(!cues?.length){setSubtitle(englishSubtitle(item));return}const update=()=>{if(!state.subtitlesOn||activeStimulus!==item)return;const time=item.subtitleElapsed?.()??elapsed,current=cues.find(cue=>cue[0]<=time&&time<cue[1]);setSubtitle(current?.[2]||"");if(time<cues.at(-1)[1])subtitleFrame=requestAnimationFrame(update)};update()}
function ensureAudio(){if(!audio)audio=new (window.AudioContext||window.webkitAudioContext)();audio.resume();return audio}
function masterOutput(){const a=ensureAudio();if(!masterBus){const limiter=a.createDynamicsCompressor();limiter.threshold.value=-3;limiter.knee.value=0;limiter.ratio.value=20;limiter.attack.value=.003;limiter.release.value=.18;masterBus=a.createGain();masterBus.gain.value=.82;masterBus.connect(limiter).connect(a.destination)}return masterBus}
function foregroundOutput(){const a=ensureAudio();if(!foregroundBus){foregroundBus=a.createGain();foregroundBus.audioClass=AUDIO_CLASS.TEXT;foregroundBus.gain.value=AUDIO_MIX.FOREGROUND;foregroundBus.connect(masterOutput())}return foregroundBus}
function soundEffectOutput(){const a=ensureAudio();if(!soundEffectBus){soundEffectBus=a.createGain();soundEffectBus.audioClass=AUDIO_CLASS.EFFECT;soundEffectBus.gain.value=SOUND_EFFECT_LEVEL*(musicDucked||audioTextActive()?AUDIO_MIX.BACKGROUND:1);soundEffectBus.connect(masterOutput())}return soundEffectBus}
...
function playPolicePassby(car){const now=performance.now();if(state.wanted<2||now<policePassbyNextAt)return;policePassbyNextAt=now+5200;const eligible=()=>state.started&&!state.modal&&!state.gameOver&&state.wanted>=2&&policeVehicles.includes(car)&&dist(player.x,player.y,car.x,car.y)<260;prepareRecording(POLICE_PASSBY_AUDIO).then(buffer=>{if(!eligible())return;const a=ensureAudio(),source=a.createBufferSource(),gain=a.createGain(),panner=a.createStereoPanner?.(),currentDistance=dist(player.x,player.y,car.x,car.y);source.buffer=buffer;source.playbackRate.value=1+Math.min(.05,(state.wanted-2)*.016);gain.gain.value=clamp(.3+(210-currentDistance)*.0015,.28,.62);source.connect(gain);if(panner){panner.pan.value=clamp((car.x-player.x)/360,-.72,.72);gain.connect(panner).connect(soundEffectOutput())}else gain.connect(soundEffectOutput());source.start()}).catch(()=>{if(eligible())playSiren()})}
...
function playTrafficHorn(car){const now=performance.now();if(now<(playTrafficHorn.nextAt||0))return;playTrafficHorn.nextAt=now+260;const a=ensureAudio(),out=a.createGain(),panner=a.createStereoPanner?.(),t=a.currentTime;out.gain.value=clamp(.035+(360-dist(player.x,player.y,car.x,car.y))*.00008,.025,.06);if(panner){panner.pan.value=clamp((car.x-player.x)/420,-.75,.75);out.connect(panner).connect(soundEffectOutput())}else out.connect(soundEffectOutput());for(const offset of [0,.19])for(const frequency of [178,224]){const o=a.createOscillator(),g=a.createGain(),start=t+offset;o.type="square";o.frequency.value=frequency+(car.kind==="trabant"?-12:8);g.gain.setValueAtTime(.0001,start);g.gain.linearRampToValueAtTime(.42,start+.012);g.gain.setValueAtTime(.42,start+.105);g.gain.exponentialRampToValueAtTime(.0001,start+.16);o.connect(g).connect(out);o.start(start);o.stop(start+.17)}car.honkFlash=.72}
...
function prepareRecording(url){if(recordingPromises.has(url))return recordingPromises.get(url);const a=ensureAudio(),promise=fetch(url).then(r=>{if(!r.ok)throw new Error("Recorded voice unavailable: "+url);return r.arrayBuffer()}).then(data=>a.decodeAudioData(data)).catch(error=>{recordingPromises.delete(url);throw error});recordingPromises.set(url,promise);return promise}
const TRAIN_PLAYER_OBSTRUCTION_AUDIO=TRAIN_ANNOUNCEMENT_AUDIO[0];
let trainAnnouncementNextAt=0;
function finishStimulus(item,generation){if(generation!==stimulusGeneration||activeStimulus!==item)return;recordedSpeechSource=null;recordedSpeechGain=null;clearSubtitle();if(item.done)item.done();if(generation!==stimulusGeneration||activeStimulus!==item)return;if(item.ambient)nextAmbientAt=performance.now()+AUDIO_MIX.AMBIENT_GAP_MS;activeStimulus=null;clearTimeout(stimulusTimer);stimulusTimer=setTimeout(()=>{for(let i=stimulusQueue.length-1;i>=0;i--)if(!stimulusEligible(stimulusQueue[i]))stimulusQueue.splice(i,1);if(stimulusQueue.length)pumpStimuli();else setAudioText()},AUDIO_MIX.REQUIRED_GAP_MS)}
function playSyntheticStimulus(item,generation){stimulusTimer=setTimeout(()=>{if(generation!==stimulusGeneration||activeStimulus!==item)return;if(item.start)item.start();if(!("speechSynthesis" in window)){finishStimulus(item,generation);return}const {text,urgent,masculine,voiceKey,lang="de-DE"}=item,u=new SpeechSynthesisUtterance(text),voices=speechSynthesis.getVoices(),languageVoices=voices.filter(v=>lang.startsWith("en")?/^en(-|_)/i.test(v.lang):/^de(-|_)/i.test(v.lang)||/german|deutsch/i.test(v.name)),h=voiceHash(voiceKey||text),railVoice=/^BAHN/.test(voiceKey||""),subtitleAuthority=/^SUBTITLE AUTHORITY/.test(voiceKey||""),male=masculine||railVoice||subtitleAuthority?languageVoices.find(v=>/conrad|markus|martin|stefan|hans|klaus|thorsten|yannick|male|männlich/i.test(v.name)):null;u.voice=male||languageVoices[h%Math.max(1,languageVoices.length)]||null;u.lang=lang;u.rate=subtitleAuthority?.72:railVoice?.76:(urgent?.96:.88)+(h%4)*.025;u.pitch=subtitleAuthority?.55:railVoice?.68:masculine?.68:(urgent?.76:.86)+(h%5)*.035;u.volume=railVoice?.94:1;let finished=false,watchdog;const finish=()=>{if(finished)return;finished=true;clearTimeout(watchdog);finishStimulus(item,generation)},arm=ms=>{clearTimeout(watchdog);watchdog=setTimeout(()=>{if(generation!==stimulusGeneration||activeStimulus!==item)return;speechSynthesis.cancel();finish()},ms)};u.onstart=()=>{item.subtitleElapsed=()=>0;startSubtitle(item);arm(Math.max(6000,text.split(/\s+/).length*900))};u.onend=finish;u.onerror=finish;arm(1800);speechSynthesis.speak(u)},AUDIO_MIX.ATTACK_SECONDS*1000)}
function playRecordedStimulus(item,generation){prepareRecording(item.recording).then(buffer=>{if(generation!==stimulusGeneration||activeStimulus!==item)return;const a=ensureAudio(),source=a.createBufferSource(),gain=a.createGain(),t=a.currentTime,ramp=Math.min(AUDIO_MIX.ATTACK_SECONDS,buffer.duration/3);source.audioClass=AUDIO_CLASS.TEXT;source.buffer=buffer;gain.gain.setValueAtTime(.0001,t);gain.gain.linearRampToValueAtTime(AUDIO_MIX.FOREGROUND,t+ramp);if(buffer.duration>ramp*2){gain.gain.setValueAtTime(AUDIO_MIX.FOREGROUND,t+buffer.duration-ramp);gain.gain.linearRampToValueAtTime(.0001,t+buffer.duration)}source.connect(gain).connect(foregroundOutput());recordedSpeechSource=source;recordedSpeechGain=gain;if(item.start)item.start();item.subtitleElapsed=()=>a.currentTime-t;source.onended=()=>finishStimulus(item,generation);startSubtitle(item);source.start()}).catch(()=>{if(item.text)playSyntheticStimulus(item,generation);else finishStimulus(item,generation)})}
...
function speak(text,{urgent=false,masculine=false,voiceKey="",start,done,lang="de-DE",family="dialogue",priority=STIMULUS_PRIORITY.CRITICAL,ambient=false,isEligible,subtitle}={}){if(!state.voiceOn){if(start)start();if(done)done();return}queueStimulus({family,priority,ambient,isEligible,text,urgent,masculine,voiceKey,start,done,lang,subtitle})}
function speakRecorded(text,recording,{urgent=false,voiceKey="",start,done,family="dialogue",priority=STIMULUS_PRIORITY.CRITICAL,ambient=false,isEligible,subtitle}={}){if(!state.voiceOn){if(start)start();if(done)done();return}queueStimulus({family,priority,ambient,isEligible,text,recording,urgent,voiceKey,start,done,subtitle})}
...
function requestTrainAnnouncement(distance,recording="",priority=STIMULUS_PRIORITY.AMBIENT){
 if(!state.voiceOn||!state.started||state.gameOver)return;const nearby=priority===STIMULUS_PRIORITY.NEARBY,direct=priority===STIMULUS_PRIORITY.CRITICAL,queuedIndex=stimulusQueue.findIndex(item=>item.family==="train");let selected=recording;
 if(queuedIndex>=0){if(stimulusQueue[queuedIndex].priority>=priority)return;selected=selected||stimulusQueue[queuedIndex].recording;stimulusQueue.splice(queuedIndex,1)}else if(activeStimulus?.family==="train"&&!nearby&&!direct)return;
 selected=selected||nextVariant("train",TRAIN_ANNOUNCEMENT_AUDIO);if(recording)rememberVariant("train",TRAIN_ANNOUNCEMENT_AUDIO,recording);if(activeStimulus?.recording===selected)return;const eligible=()=>state.started&&!state.modal&&!state.gameOver&&nearestTrainDistance()<(nearby?TRAIN_NEARBY_AUDIO_RADIUS:TRAIN_AUDIO_RADIUS);
 queueStimulus({family:"train",priority,ambient:priority===STIMULUS_PRIORITY.AMBIENT,isEligible:priority===STIMULUS_PRIORITY.CRITICAL?undefined:eligible,recording:selected,done:()=>{const stillNearby=nearby&&eligible();trainAnnouncementNextAt=performance.now()+(stillNearby?0:3500);if(stillNearby)requestTrainAnnouncement(nearestTrainDistance(),"",STIMULUS_PRIORITY.NEARBY)}})
}
function updateTrainAnnouncement(){const nearest=nearestTrainDistance(),nearby=nearest<TRAIN_NEARBY_AUDIO_RADIUS,priority=nearby?STIMULUS_PRIORITY.NEARBY:STIMULUS_PRIORITY.AMBIENT;if(nearest<TRAIN_AUDIO_RADIUS&&state.started&&!state.modal&&!state.gameOver&&state.voiceOn&&(nearby||performance.now()>=trainAnnouncementNextAt))requestTrainAnnouncement(nearest,"",priority)}
...
function updateWirtschaftswunderCar(car,dt){
 ...
 if(car.vortexPhase==="approach"){
  const previousX=car.x,previousY=car.y,t=car.vortexT=Math.min(1,car.vortexT+dt/2.65),endX=site.x+site.radius*.92,endY=site.y+site.radius*.2;
  car.x=cubicPoint(car.vortexStartX,site.entryX+car.dir*90,site.x+site.radius*1.55,endX,t);car.y=cubicPoint(car.vortexStartY,car.vortexStartY-210,site.y+site.radius*1.05,endY,t);car.angle=Math.atan2(car.y-previousY,car.x-previousX);
  if(t===1){car.vortexPhase="spiral";car.vortexT=0;car.vortexAngle=.215;car.vortexSink=0}return
 }
...
function updateTraffic(dt){
 for(const car of trafficCars){
  ...
  car.blockedByPlayer=playerGap<=queueGap+.01&&playerGap<TRAFFIC_BRAKE_DISTANCE;car.queued=advance+.01<intended||obstacleGap<TRAFFIC_BRAKE_DISTANCE;
  if(car.blockedByPlayer&&car.speed<30){car.hold+=dt;if(car.hold>.34&&car.honkCooldown===0){playTrafficHorn(car);car.honkCooldown=1.35+Math.random()*2.15}}else car.hold=Math.max(0,car.hold-dt*2.5)
 }
}
...
function proximityAudioReady(n){
 ...
 if(near>release){if(n.dialogueNearby)n.barkAt=0;n.dialogueNearby=false;n.featuredAudioActive=false;return false}
 if(near>=radius)return false;
 n.dialogueNearby=true;return true
}
function featuredSpriteEligible(n){const germanyOnly=n.special==="bayern"||n.special==="alice";return state.started&&!state.modal&&!state.gameOver&&(!germanyOnly||state.region==="germany")&&dist(player.x,player.y,n.x,n.y)<(n.audioRadius||SPRITE_AUDIO_RADIUS)}
function showFeaturedSpriteBark(n,family,item,nextItem){
 const line=typeof item==="string"?{text:item}:item;if(n.featuredAudioActive||!line?.text||!featuredSpriteEligible(n))return false;const eligible=()=>featuredSpriteEligible(n);n.barkAt=0;n.featuredAudioActive=true;
 showWorldBark(n.name,line.text,false,line.recording||"","",{family,priority:STIMULUS_PRIORITY.NEARBY,ambient:false,isEligible:eligible,done:()=>{n.featuredAudioActive=false;if(eligible())showFeaturedSpriteBark(n,family,nextItem(),nextItem)}});return true
}
function featuredSpriteFamily(n){return n.special==="merkel"?"politician:merkel":n.special==="borderPourer"?"politician:merz":n.special}
function nearbyFeaturedSprite(){return npcs.find(n=>n.special&&featuredSpriteEligible(n))||null}
...
function bayernBark(n,force=false){const now=performance.now();if((!force&&hasStimulusFamily("bayern"))||state.region!=="germany")return false;const item=nextBayernClip();if(!force)return showFeaturedSpriteBark(n,"bayern",item,nextBayernClip);n.barkAt=now+9000;showWorldBark(n.name,item.text,false,item.recording,"",{family:"bayern",priority:STIMULUS_PRIORITY.CRITICAL,ambient:false});return true}
...
function aliceBark(n,force=false){
 const now=performance.now();if((!force&&hasStimulusFamily("alice"))||state.region!=="germany")return false;const item=nextAliceClip();n.barkAt=now+item.duration*1000+2500;
 if(!force)return showFeaturedSpriteBark(n,"alice",item,nextAliceClip);showWorldBark(n.name,item.text,false,item.recording,"",{family:"alice",priority:STIMULUS_PRIORITY.CRITICAL,ambient:false});return true
}
...
function showQuizPerson(person){
 const frame=document.querySelector(".quiz-portrait-frame"),image=document.getElementById("quiz-portrait");frame.classList.remove("missing");image.alt=person.alt||person.name;image.onload=()=>frame.classList.remove("missing");image.onerror=()=>frame.classList.add("missing");if(person.portrait)image.src=person.portrait;else{image.removeAttribute("src");frame.classList.add("missing")}
 document.getElementById("quiz-person-name").textContent=person.name;document.getElementById("quiz-person-title").textContent=person.title;document.getElementById("quiz-person-office").textContent=person.office;document.getElementById("quiz-person-authority").textContent=person.authority;document.getElementById("quiz-person-specialty").textContent=person.specialty
}
function startCitizenshipQuiz(n){
 ...
 showQuizPerson(character);document.getElementById("quiz-speaker").textContent=isDriving?"SPONTANE FAHRSCHUL-QUERPRÜFUNG":isGrammar?"SPONTANE "+q.level+"-GRAMMATIK-QUERPRÜFUNG":"SPONTANE EINBÜRGERUNGSPRÜFUNG";document.getElementById("quiz-source").textContent=isDriving?"FIKTIVE SPIELFRAGE · "+q.source:isGrammar?"ZERTIFIKATSNAHE SPIELÜBUNG · KEIN ECHTER PRÜFUNGSSATZ · GER "+q.level+" · "+q.source:"BAMF-GESAMTKATALOG 07.05.2025 · AUFGABE "+q.source;document.getElementById("quiz-context").textContent=context;document.getElementById("quiz-prompt").textContent=copy.question;choices.innerHTML="";
...
function updateQuizEncounters(dt){
 ...
 if(candidates.length){candidates.sort((a,b)=>dist(player.x,player.y,a.x,a.y)-dist(player.x,player.y,b.x,b.y));state.quizApproach=pick(candidates.slice(0,Math.min(6,candidates.length)));state.quizApproach.quizFollowTime=0}
...
function renderDialogue(){const d=state.dialogueData,line=d.lines[d.i],borderMan=d.speaker.startsWith("FRIEDRICH MERZ"),memorial=d.portrait==="KG",dialogue=document.getElementById("dialogue");dialogue.classList.toggle("memorial",memorial);dialogue.hidden=state.voiceOn&&!memorial;document.getElementById("speaker").textContent=d.speaker;document.getElementById("portrait").textContent=d.portrait;document.getElementById("dialogue-text").textContent=line;document.getElementById("dialogue-next").textContent=d.i===d.lines.length-1?(state.lang==="en"?"UNDERSTOOD":"VERSTANDEN"):(state.lang==="en"?"CONTINUE":"WEITER");document.getElementById("voice-state").textContent=state.voiceOn?(memorial?"HISTORISCHER TEXT · DEUTSCH":borderMan?"COMPUTERSTIMME · MÄNNLICH · NUR DEUTSCH":state.region==="berlin"?"COMPUTERSTIMME · DENG-LISCH":"COMPUTERSTIMME · NUR DEUTSCH"):(state.lang==="en"?"VOICE OFF":"STIMME AUS");if(memorial)dialogue.scrollTop=0;speakDialogueLine(line,()=>dialogue.hidden=false)}
...
function update(dt){
 ...
 if(player.energy<22&&!state.lowEnergyRemarked){state.lowEnergyRemarked=true;innerMonologue("exhaustion")}else if(player.energy>38)state.lowEnergyRemarked=false;
 ...
 collect();hudRefreshTimer-=dt;if(hudRefreshTimer<=0){hudRefreshTimer=HUD_REFRESH_SECONDS;updateHud()}
}
...
function nearestInteract(){let label="",best=122;const memorialDistance=dist(player.x,player.y,kiesingerMemorial.x,kiesingerMemorial.y+195);if(memorialDistance<best){best=memorialDistance;label="KIESINGER-DENKMAL LESEN"}for(const b of buildings){const d=dist(player.x,player.y,b.doorX,b.doorY);if(d<best){best=d;label=b.name}}for(const n of npcs){const d=dist(player.x,player.y,n.x,n.y);if(d<best){best=d;label=n.special==="borderPourer"||n.special==="alice"?(state.region==="berlin"?"SATIRE STATEMENT LISTENING":"SATIRISCHE ERKLÄRUNG ANHÖREN"):(state.region==="berlin"?"COMPLAINT LISTENING":"BESCHWERDE ANHÖREN")}}for(const p of props){if(!p.id)continue;const d=dist(player.x,player.y,p.x,p.y);if(d<best){best=d;label=p.label}}for(const o of normObjects){if(o.fixed)continue;const d=dist(player.x,player.y,o.x,o.y);if(d<best){best=d;label="AUSRICHTEN"}}const e=document.getElementById("interact-hint");e.hidden=!label;e.textContent=label?"E · "+label:""}
...
let audio=null,musicTimer=null,musicBus=null,soundEffectBus=null,musicOn=true,musicDucked=false,currentTune=-1,forcedTune=-1,introMusicFinished=false,introRampFrame=0,sungRampFrame=0,sungMusicActive=false,eightBitTracksSinceSong=0,wurstSongPending=false,wurstSongPlayed=false,wurstTransitioning=false,musicPlaybackSerial=0;
const INTRO_MUSIC_AUDIO="./assets/intro-song.mp3";
const WURST_MUSIC=Object.freeze({title:"Wurst",src:"./assets/audio/music/wurst.mp3"});
...
function playSungMusic(song=SUNG_MUSIC[nextVariant("sung-music",SUNG_MUSIC_PLAYLIST)]){const serial=++musicPlaybackSerial;sungMusicActive=true;eightBitTracksSinceSong=0;sungMusicAudio.src=song.src;sungMusicAudio.volume=0;document.getElementById("mute").title=`NOW PLAYING · ${song.title}`;Promise.resolve(sungMusicAudio.play()).then(()=>{if(serial!==musicPlaybackSerial)return;rampSungVolume(SUNG_MUSIC_LEVEL*(musicDucked||audioTextActive()?AUDIO_MIX.BACKGROUND:1),.12);if(song===WURST_MUSIC){wurstSongPending=false;wurstSongPlayed=true;wurstTransitioning=false}}).catch(()=>{if(serial!==musicPlaybackSerial)return;if(song===WURST_MUSIC)wurstTransitioning=false;finishSungMusic()})}
function playWurstSong(){if(!musicOn||!wurstSongPending||wurstTransitioning)return;wurstTransitioning=true;clearTimeout(musicTimer);const oldBus=musicBus;if(oldBus){rampGain(oldBus.gain,0,.12);musicBus=null;setTimeout(()=>oldBus.disconnect(),160)}if(!introMusicFinished)rampIntroVolume(0,.12);if(sungMusicActive)rampSungVolume(0,.12);setTimeout(()=>{if(!musicOn){wurstTransitioning=false;return}musicPlaybackSerial++;stopIntroMusic();sungMusicAudio.pause();sungMusicActive=false;playSungMusic(WURST_MUSIC)},160)}
function queueWurstSong(){if(wurstSongPlayed||wurstSongPending)return;wurstSongPending=true;playWurstSong()}
...
function startMusic(event){
 if(!musicOn)return;if(event){if(!audio)audio=new (window.AudioContext||window.webkitAudioContext)();audio.resume()}if(wurstSongPending){playWurstSong();return}if(!introMusicFinished){introMusicAudio.play().catch(()=>{});return}if(sungMusicActive){removeMusicUnlockListeners();sungMusicAudio.play().catch(()=>{});return}if(musicBus)return;if(!audio)audio=new (window.AudioContext||window.webkitAudioContext)();audio.resume();musicBus=createMusicBus();removeMusicUnlockListeners();schedulePlaylistItem()
}
...
document.getElementById("mute").onclick=()=>{musicOn=!musicOn;document.getElementById("mute").textContent=musicOn?"MUSIC ON":"MUSIC OFF";if(musicOn)startMusic();else{musicPlaybackSerial++;wurstTransitioning=false;clearTimeout(musicTimer);stopIntroMusic();sungMusicAudio.pause();sungMusicActive=false;musicBus?.disconnect();musicBus=null}};
const humorPages=[
...
let faxFeedCue=0;
function beginHumorFax(done){const cue=++faxFeedCue;humorAudio.className="humor-audio processing";document.getElementById("humor-audio-status").textContent=humorFormLanguage==="en"?"SHEET IS BEING TRANSMITTED BY FAX":"BLATT WIRD PER FAX EINGEZOGEN";humorModal.classList.add("fax-processing");humorForm.classList.add("fax-feeding");playFaxFeed(()=>cue===faxFeedCue&&humorModal.classList.contains("fax-processing"));setTimeout(()=>{humorForm.classList.remove("fax-feeding");humorModal.classList.remove("fax-processing");humorSubmitting=false;done()},1120)}
...
const SUBTITLE_APPROVAL_LINES=["Englische Untertitel sind äußerst wichtig, insbesondere wenn Sie Deutsch lernen möchten.","Aufgrund Ihres bemerkenswerten Lerneifers wird die Verwendung von Untertiteln hiermit genehmigt.","Wir gratulieren Ihnen zu dieser verwaltungstechnisch ausgezeichneten Entscheidung."];
try{state.subtitlesOn=localStorage.getItem("germany-simulator-english-subtitles")==="on"}catch{}
function updateSubtitleToggle(){const button=document.getElementById("subtitle-toggle");button.textContent=state.subtitlesOn?"EN SUBTITLES ON":"EN SUBTITLES OFF";button.setAttribute("aria-pressed",String(state.subtitlesOn));button.title=state.subtitlesOn?"Disable English subtitles":"Enable English subtitles"}
updateSubtitleToggle();
document.getElementById("subtitle-toggle").onclick=()=>{state.subtitlesOn=!state.subtitlesOn;try{localStorage.setItem("germany-simulator-english-subtitles",state.subtitlesOn?"on":"off")}catch{}updateSubtitleToggle();if(!state.subtitlesOn)clearSubtitle();else{if(activeStimulus)startSubtitle(activeStimulus,activeStimulus.subtitleElapsed?.()||0);if(!hasStimulusFamily("subtitle-approval"))for(const text of SUBTITLE_APPROVAL_LINES)speak(text,{voiceKey:"SUBTITLE AUTHORITY",family:"subtitle-approval",isEligible:()=>state.subtitlesOn})}};
document.getElementById("voice-toggle").onclick=()=>{state.voiceOn=!state.voiceOn;document.getElementById("voice-toggle").textContent=state.voiceOn?"VOICE ON":"VOICE OFF";if(!state.voiceOn){stopSpeech(true);state.dialogueVoiceToken=(state.dialogueVoiceToken||0)+1;setDialogueVoiceBusy(false)}};
 function keydown(e){if(["ArrowUp","ArrowDown","ArrowLeft","ArrowRight"].includes(e.code))e.preventDefault();keys[e.code]=true;if(e.repeat)return;if(e.code==="KeyS"&&!state.started&&!document.getElementById("intro").classList.contains("hidden")){startGame(true);return}if(e.code==="KeyE")interact();if(e.code==="KeyQ"||e.key==="§")useLawPower()}function keyup(e){keys[e.code]=false}function clearInput(){for(const code in keys)keys[code]=false}addEventListener("keydown",keydown);addEventListener("keyup",keyup);addEventListener("blur",clearInput);document.addEventListener("visibilitychange",()=>{last=performance.now();if(document.hidden)clearInput()});
...
function loop(now){const dt=Math.min(.12,Math.max(0,(now-last)/1000));last=now;if(dt){const steps=Math.ceil(dt/.025);for(let i=0;i<steps;i++)update(dt/steps)}updatePoliceChaseAudio();draw();window.Germany3D?.sync();requestAnimationFrame(loop)}updateHud();requestAnimationFrame(loop);
})();
