...
const merkelLines=["Wir schaffen das.","Das Internet ist für uns alle Neuland.","Ausspähen unter Freunden, das geht gar nicht.","Scheitert der Euro, dann scheitert Europa."];
const npcs=[
...
function playQueuedSpeech(){if(speechActive||!speechQueue.length||!state.voiceOn)return;speechActive=true;const {text,urgent,masculine,done}=speechQueue.shift(),generation=speechGeneration,u=new SpeechSynthesisUtterance(text),voices=speechSynthesis.getVoices(),german=voices.filter(v=>/^de(-|_)/i.test(v.lang)||/german|deutsch/i.test(v.name));u.voice=(masculine?german.find(v=>/conrad|markus|martin|stefan|hans|klaus|thorsten|yannick|male|männlich/i.test(v.name)):null)||german[0]||null;u.lang="de-DE";u.rate=urgent?.98:.9;u.pitch=masculine?.68:urgent?.72:.88;u.volume=1;let finished=false;const finish=()=>{if(finished||generation!==speechGeneration)return;finished=true;if(done)done();speechPauseTimer=setTimeout(()=>{speechActive=false;playQueuedSpeech()},SPEECH_GAP_MS)};u.onend=finish;u.onerror=finish;speechSynthesis.speak(u)}
function speak(text,{urgent=false,masculine=false,done}={}){if(!state.voiceOn||!("speechSynthesis" in window)){if(done)done();return}speechQueue.push({text,urgent,masculine,done});playQueuedSpeech()}
...
function speakDialogueLine(line){const token=state.dialogueVoiceToken=(state.dialogueVoiceToken||0)+1,masculine=state.dialogueData?.speaker?.startsWith("FRIEDRICH MERZ");setDialogueVoiceBusy(state.voiceOn&&"speechSynthesis" in window);speak(line,{masculine,done:()=>{if(token===state.dialogueVoiceToken)setDialogueVoiceBusy(false)}})}
