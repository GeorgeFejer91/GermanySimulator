...
let humorPageIndex=0,humorReadComplete=false,humorActionStep=0,humorFormLanguage="de";
...
humorRepeat.onclick=readHumorPage;humorForm.addEventListener("input",updateHumorContinue);humorForm.onsubmit=event=>{event.preventDefault();if(!humorReadComplete||humorActionStep<activeHumorPage().buttons.length||!humorFieldsValid()){humorForm.reportValidity();updateHumorContinue();return}if(++humorPageIndex<humorPages.length)renderHumorPage();else finishHumorCertification()};
