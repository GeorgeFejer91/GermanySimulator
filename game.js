...
const WURST_TYPES=Object.freeze({
 nuernberger:{label:"NÜRNBERGER ROSTBRATWURST",short:"NÜRNBERGER",mark:"N",color:"#b9845f",pieces:3},
 frankfurter:{label:"FRANKFURTER WÜRSTCHEN",short:"FRANKFURTER",mark:"F",color:"#9d5a49",pieces:2},
 thueringer:{label:"THÜRINGER ROSTBRATWURST",short:"THÜRINGER",mark:"T",color:"#a97550",pieces:1},
 weisswurst:{label:"MÜNCHNER WEISSWURST",short:"WEISSWURST",mark:"W",color:"#d8c7aa",pieces:2},
 bockwurst:{label:"BOCKWURST",short:"BOCKWURST",mark:"B",color:"#a34e3f",pieces:1},
 knackwurst:{label:"KNACKWURST",short:"KNACKWURST",mark:"K",color:"#8b493b",pieces:2},
 mettwurst:{label:"METTWURST",short:"METTWURST",mark:"M",color:"#93463c",pieces:1},
 teewurst:{label:"TEEWURST",short:"TEEWURST",mark:"TW",color:"#b2614b",pieces:1},
 currywurstBadge:{label:"BERLINER CURRYWURST",short:"CURRYWURST",mark:"C",color:"#bd563f",pieces:4}
});
...
const pedestrianBarks={
 ...
 germany:{
  sidewalk:["HALT! Sie stehen im normierten Gehwegprofil!","STOPP! Sie blockieren den Durchgang nach meiner Auslegung!","Sie sind im Weg. Ich war verwaltungsrechtlich zuerst hier!","Hast du schon Kehrwoche gemacht? Dann kennen Sie doch die Gehwegordnung!","Das Leben ist kein Ponyhof. Verlassen Sie meine Lauflinie!","Haben Sie Tomaten auf den Augen? Das ist eindeutig mein Gehweg!",{text:"Nein, ich gehe hier nicht weg!",recording:"./assets/voices/thorsten-angry-nicht-weg.mp3"},{text:"Dümmer geht's nicht mehr.",recording:"./assets/voices/thorsten-angry-duemmer.mp3"},{text:"Das klappt einfach nicht!",recording:"./assets/voices/thorsten-angry-klappt-nicht.mp3"}],
...
function renderWurstBadges(){
 ...
 for(const id of WURST_IDS){const def=WURST_TYPES[id],badge=document.createElement("span");badge.className="wurst-badge"+(state.wurstBadges.has(id)?" collected":"");badge.textContent=def.mark;badge.title=def.label;badge.style.setProperty("--wurst-color",def.color);list.appendChild(badge)}
 document.getElementById("wurst-count").textContent=state.wurstBadges.size+"/"+WURST_IDS.length
}
function announceWurst(def){const box=document.getElementById("wurst-alert");document.getElementById("wurst-alert-text").textContent="UNLOCKED "+def.label+" · +1 GERMANNESS";box.hidden=false;box.classList.remove("show");void box.offsetWidth;box.classList.add("show");clearTimeout(announceWurst.t);announceWurst.t=setTimeout(()=>{box.hidden=true;box.classList.remove("show")},2800)}
...
function useLawPower(){
 ...
 const quote=pick(lawPowerLines);state.lawCooldown=14;state.wanted=0;state.wantedCooldown=0;state.offence="ZUSTÄNDIGKEIT ERFOLGREICH UMGELENKT";
 ...
 showWorldBark("SIE · GESETZZITAT",quote,true);toast("§-MACHT AKTIV · "+target.name+" WIRD ÜBERPRÜFT");updateHud();
}
...
function updateHud(){
 ...
 const r=rules[state.rule%rules.length];document.getElementById("rule-id").textContent=r[0];document.getElementById("rule-text").textContent=localize(r[1]);document.getElementById("region-name").textContent=state.region==="berlin"?"BERLIN":"DEUTSCHLAND";document.getElementById("region-language").textContent=state.region==="berlin"?"Denglisch-Zone":"Nur Deutsch";
...
let audio=null,musicTimer=null,musicBus=null,musicOn=true,musicBag=[],currentTune=-1,forcedTune=-1;
...
function queueNationalAnthem(){forcedTune=1;if(!musicOn||!audio)return;clearTimeout(musicTimer);musicBus?.disconnect();musicBus=audio.createGain();musicBus.connect(audio.destination);scheduleTheme()}
...
const humorPages=[
 {title:"1. Begriffsprüfung: Humor",text:"Dieses fiktive Spielformular dokumentiert ausschließlich die Kenntnisnahme eines kommunikativen Phänomens und ist keine Rechtsberatung. Humor bezeichnet hier eine erkennbare Abweichung zwischen dem wörtlich Erwartbaren und dem tatsächlich Dargestellten. Diese Abweichung kann durch Witz, Ironie, Übertreibung, Untertreibung, Absurdität, Nachahmung, falsche Feierlichkeit oder absichtlich bürokratische Umständlichkeit entstehen. Sie kann Belustigung, Überraschung, Unbehagen, Kritik oder liebevolle Verspottung bezwecken. Ob eine bestimmte Person lacht, entscheidet nicht darüber, ob Humor beabsichtigt war. Ein misslungener, langweiliger oder geschmacklich abgelehnter Witz kann weiterhin als humoristische Kommunikation gemeint sein. Mit der Unterschrift wird weder Geschmack noch Zulässigkeit bescheinigt, sondern nur die Möglichkeit nichtwörtlicher Bedeutung anerkannt.",fields:[{type:"text",label:"PARAPHE DER ANTRAGSTELLENDEN PERSON",minLength:2},{type:"check",label:"Ich bestätige, dass Humor als kommunikative Möglichkeit existiert."}],buttons:["WORTLAUT GESEHEN","NICHTLACHEN PROTOKOLLIEREN"]},
 {title:"2. Satire und verwandte Verfahren",text:"Satire bezeichnet eine Darstellung, die Personen, Institutionen, Gewohnheiten oder Verfahren durch bewusste Verfremdung für Kritik, Spott, Kommentar oder komische Distanz umformt. Ironie lässt den wörtlichen Satz und die im Zusammenhang gemeinte Aussage auseinanderfallen. Sarkasmus ist eine zugespitzte, häufig bissige Form ironischer Rede. Parodie ruft ein bekanntes Werk, einen Stil oder eine Konvention erkennbar auf und verändert sie für Humor oder Kritik. Karikatur betont, vereinfacht oder verzerrt erkennbare Merkmale unverhältnismäßig. Übertreibung steigert Größe, Häufigkeit, Sicherheit oder Folgen absichtlich über ein gewöhnliches Maß hinaus. Absurdität führt unlogische, unmögliche, unverhältnismäßige oder verfahrensmäßig übertriebene Elemente ein. Deadpan-Darstellung trägt komischen Inhalt vollständig ernst, sachlich oder amtlich vor. Ein erfundenes Bundesamt für Spontaneität, das spontane Handlungen vierzehn Tage vorher verlangt, wird durch DIN-gerechte Typografie nicht zu einer realen Behörde.",fields:[{type:"text",label:"GEGENZEICHNUNG NACH VOLLSTÄNDIGER ANHÖRUNG",minLength:2},{type:"check",label:"Ich kann Satire, Ironie, Parodie, Karikatur, Übertreibung, Absurdität und Deadpan mindestens theoretisch auseinanderhalten."}],buttons:["SATIRE ERKANNT","WÖRTLICHKEIT VERNEINT","TÜV-BEFREIUNG BEANTRAGEN"]},
 {title:"3. Kontextprüfung und Schlusserklärung",text:"Humoristische Bedeutung wird im Zusammenhang beurteilt. Hinweise können Titel, Genre, Bildsprache, offenkundige Unmöglichkeit, innere Widersprüche, Übertreibung, bekannte Anspielungen, fiktive Sprecher und die Konventionen von Parodie oder Karikatur sein. Die förmliche Erscheinung eines Textes mit Aktenzeichen, Kästchen, Stempelfeld und Unterschriftslinie beweist nicht, dass eine echte Behörde ihn erlassen hat. Humor zu erkennen bedeutet nicht, ihn gutzuheißen. Eine Person darf einen Witz kindisch, beleidigend, langweilig, unfair oder katastrophal deutsch finden und trotzdem seine nichtwörtliche Absicht erkennen. Eine offensichtlich erfundene Handlung einer karikierten öffentlichen Figur ist nicht automatisch eine Tatsachenbehauptung über die reale Person. Ich erkläre daher: Ich weiß grundsätzlich, was Humor ist, und erwäge vor Einleitung eines parlamentarischen Untersuchungsausschusses die Möglichkeit, dass das Lächerliche mit Absicht lächerlich sein könnte.",fields:[{type:"text",label:"NAME IN DRUCKBUCHSTABEN",minLength:2},{type:"text",label:"UNTERSCHRIFT DER ANTRAGSTELLENDEN PERSON",minLength:2},{type:"text",label:"GEGENUNTERSCHRIFT EINER PERSON, DIE DIE UNTERSCHRIFT BESTÄTIGT",minLength:2},{type:"text",label:"ORT DER HUMORKENNTNISNAHME",minLength:2},{type:"date",label:"DATUM"},{type:"check",label:"Ich verstehe, dass nicht jede Aussage wörtlich gemeint ist."},{type:"check",label:"Ich verstehe, dass ein Witz nicht durch mein persönliches Nichtlachen rückwirkend zur Tatsachenbehauptung wird."},{type:"check",label:"Ich erkenne an, dass dieses Formular selbst Teil des Witzes und kein echter Vertrag ist."}],buttons:["UNTERSCHRIFTEN AUF VOLLSTÄNDIGKEIT PRÜFEN","ANTRAG ZUM ANTRAG FREIGEBEN"]}
];
const humorModal=document.getElementById("humor-modal"),humorForm=document.getElementById("humor-form"),humorNext=document.getElementById("humor-next"),humorRepeat=document.getElementById("humor-repeat"),humorAudio=document.querySelector(".humor-audio");
let humorPageIndex=0,humorReadComplete=false,humorActionStep=0;
function humorFieldsValid(){return humorForm.checkValidity()}
function updateHumorContinue(){const page=humorPages[humorPageIndex],actionsComplete=humorActionStep>=page.buttons.length,fieldsComplete=humorFieldsValid(),ready=humorReadComplete&&actionsComplete&&fieldsComplete;humorNext.disabled=!ready;humorNext.textContent=!humorReadComplete?"VORLESUNG ABWARTEN":!fieldsComplete?"ALLE FELDER AUSFÜLLEN":!actionsComplete?"ALLE AMTSKNÖPFE DRÜCKEN":humorPageIndex===humorPages.length-1?"HUMORKENNTNIS BESCHEINIGEN":"NÄCHSTES BLATT";document.getElementById("humor-error").textContent=ready?"":"FORTSETZUNG ERST NACH VOLLSTÄNDIGER ANHÖRUNG UND BEARBEITUNG"}
function renderHumorRequirements(page){const wrap=document.getElementById("humor-requirements");wrap.innerHTML="";for(const [index,field] of page.fields.entries()){const label=document.createElement("label"),input=document.createElement("input"),span=document.createElement("span");input.type=field.type==="check"?"checkbox":field.type;input.required=true;if(field.minLength)input.minLength=field.minLength;span.textContent=field.label;label.className=field.type==="check"?"check":"humor-signature";if(field.type==="check")label.append(input,span);else label.append(span,input);wrap.append(label)}const actions=document.createElement("div"),buttons=[];actions.className="humor-actions";page.buttons.forEach((text,index)=>{const button=document.createElement("button");button.type="button";button.textContent=(index+1)+" · "+text;button.disabled=index!==0;button.onclick=()=>{if(index!==humorActionStep)return;button.disabled=true;button.classList.add("stamped");button.textContent="ABGESTEMPELT · "+text;humorActionStep++;if(buttons[index+1])buttons[index+1].disabled=false;uiTone(92,.07,"square",.025);updateHumorContinue()};buttons.push(button);actions.append(button)});wrap.append(actions)}
function readHumorPage(){stopSpeech();const page=humorPages[humorPageIndex],spoken=page.title+". "+page.text,chunks=spoken.match(/[^.!?]+[.!?]+|[^.!?]+$/g)||[spoken];humorReadComplete=false;humorAudio.className="humor-audio reading";humorRepeat.disabled=true;document.getElementById("humor-audio-status").textContent="BLATT "+(humorPageIndex+1)+" WIRD VOLLSTÄNDIG VORGELESEN";updateHumorContinue();chunks.forEach((chunk,index)=>speak(chunk.trim(),{voiceKey:"HUM-01/DE VORLESESTELLE",done:index===chunks.length-1?()=>{humorReadComplete=true;humorAudio.className="humor-audio complete";humorRepeat.disabled=false;document.getElementById("humor-audio-status").textContent="VORLESUNG VOLLSTÄNDIG BEENDET";updateHumorContinue()}:undefined}))}
function renderHumorPage(){stopSpeech();const page=humorPages[humorPageIndex];humorActionStep=0;document.getElementById("humor-page-label").textContent="BLATT "+(humorPageIndex+1)+" VON "+humorPages.length;document.getElementById("humor-title").textContent=page.title;document.getElementById("humor-readout").textContent=page.text;renderHumorRequirements(page);humorForm.scrollTop=0;updateHumorContinue();readHumorPage()}
function finishHumorCertification(){stopSpeech();humorModal.hidden=true;state.started=true;state.modal=false;startMusic();const lines=state.region==="berlin"?["Welcome in Berlin. Hier reden wir erstmal practical Denglisch.","Your mission ist simple: become German citizen in drei Behördentagen.","Aber careful: more than three seconds auf grass or street gibt einen Polizeistern. Use the Zebrastreifen.","Berlin liegt hinter der Brandmauer. You can cross sie freely."]:["Willkommen in Deutschland.","Ihr Ziel: Werden Sie innerhalb von drei völlig fiktiven Behördentagen deutscher Staatsbürger.","Dazu benötigen Sie vor allem Formulare. Sehr viele Formulare.","Wer länger als drei Sekunden auf Rasen oder Straße bleibt, erhält einen Polizeistern. Benutzen Sie den Zebrastreifen.","Berlin liegt hinter der Brandmauer. Sie ist frei überquerbar."];openDialogue(state.region==="berlin"?"WELCOME TO BERLIN":"WILLKOMMEN IN DEUTSCHLAND",lines,"DE",()=>toast("ERSTER VORGANG · BÜRGERAMT"));updateHud()}
function startGame(){ensureAudio();prepareMerkelRecording().catch(()=>{});state.region=regionOf(player.y);state.modal=true;document.getElementById("intro").classList.add("hidden");humorModal.hidden=false;humorPageIndex=0;renderHumorPage()}
humorRepeat.onclick=readHumorPage;humorForm.addEventListener("input",updateHumorContinue);humorForm.onsubmit=event=>{event.preventDefault();if(!humorReadComplete||humorActionStep<humorPages[humorPageIndex].buttons.length||!humorFieldsValid()){humorForm.reportValidity();updateHumorContinue();return}if(++humorPageIndex<humorPages.length)renderHumorPage();else finishHumorCertification()};
