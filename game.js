...
const SIDEWALK_WIDTH=72,WALKWAY_WIDTH=84,NPC_BLOCK_DISTANCE=38,NPC_COMPLAINT_DISTANCE=82,SPRITE_AUDIO_RADIUS=240,SPRITE_AUDIO_RELEASE_RADIUS=310,QUIZ_FOLLOW_MAX_SECONDS=8,QUIZ_FOLLOW_BREAK_DISTANCE=650;
...
function playSyntheticStimulus(item,generation){stimulusTimer=setTimeout(()=>{if(generation!==stimulusGeneration||activeStimulus!==item)return;if(item.start)item.start();if(!("speechSynthesis" in window)){finishStimulus(item,generation);return}const {text,urgent,masculine,voiceKey,lang="de-DE"}=item,u=new SpeechSynthesisUtterance(text),voices=speechSynthesis.getVoices(),languageVoices=voices.filter(v=>lang.startsWith("en")?/^en(-|_)/i.test(v.lang):/^de(-|_)/i.test(v.lang)||/german|deutsch/i.test(v.name)),h=voiceHash(voiceKey||text),railVoice=/^BAHN/.test(voiceKey||""),male=masculine||railVoice?languageVoices.find(v=>/conrad|markus|martin|stefan|hans|klaus|thorsten|yannick|male|männlich/i.test(v.name)):null;u.voice=male||languageVoices[h%Math.max(1,languageVoices.length)]||null;u.lang=lang;u.rate=railVoice?.76:(urgent?.96:.88)+(h%4)*.025;u.pitch=railVoice?.68:masculine?.68:(urgent?.76:.86)+(h%5)*.035;u.volume=railVoice?.94:1;let finished=false,watchdog;const finish=()=>{if(finished)return;finished=true;clearTimeout(watchdog);finishStimulus(item,generation)},arm=ms=>{clearTimeout(watchdog);watchdog=setTimeout(()=>{if(generation!==stimulusGeneration||activeStimulus!==item)return;speechSynthesis.cancel();finish()},ms)};u.onstart=()=>arm(Math.max(6000,text.split(/\s+/).length*900));u.onend=finish;u.onerror=finish;arm(1800);speechSynthesis.speak(u)},AUDIO_MIX.ATTACK_SECONDS*1000)}
