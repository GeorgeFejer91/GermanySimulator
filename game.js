...
const AUDIO_CLASS=Object.freeze({TEXT:"audio-text",BACKGROUND:"background-music",EFFECT:"sound-effect"}),AUDIO_TEXT_PRIORITY=Object.freeze({DIALOGUE:1,TRAIN:2,TRAIN_OBSTRUCTION:3});
let activeAudioText=null;
function ensureAudio(){if(!audio)audio=new (window.AudioContext||window.webkitAudioContext)();audio.resume();return audio}
function soundEffectOutput(){const a=ensureAudio();if(!soundEffectBus){soundEffectBus=a.createGain();soundEffectBus.audioClass=AUDIO_CLASS.EFFECT;soundEffectBus.gain.value=audioTextActive()?SOUND_EFFECT_READOUT_LEVEL:SOUND_EFFECT_LEVEL;soundEffectBus.connect(a.destination)}return soundEffectBus}
function audioTextActive(){return !!activeAudioText}
function setAudioText(owner=null,priority=AUDIO_TEXT_PRIORITY.DIALOGUE){activeAudioText=owner?{className:AUDIO_CLASS.TEXT,owner,priority}:null;applyAudioDucking()}
...
function playFaxFeed(){prepareRecording(FAX_FEED_AUDIO).then(buffer=>{if(faxFeedSource)try{faxFeedSource.stop()}catch{}const a=ensureAudio(),source=a.createBufferSource(),gain=a.createGain();faxFeedSource=source;source.buffer=buffer;gain.gain.value=.55;source.connect(gain).connect(soundEffectOutput());source.onended=()=>{if(faxFeedSource===source)faxFeedSource=null};source.start()}).catch(playSynthFaxFeed)}
...
function finishSpeechItem(done,generation){if(generation!==speechGeneration)return;if(done)done();speechPauseTimer=setTimeout(()=>{speechActive=false;if(trainAnnouncementPending){playTrainAnnouncement();return}if(speechQueue.length){playQueuedSpeech();return}setAudioText()},SPEECH_GAP_MS)}
...
let trainAnnouncementIndex=Math.floor(Math.random()*TRAIN_ANNOUNCEMENT_AUDIO.length),trainAnnouncementSource=null,trainAnnouncementGain=null,trainAnnouncementLoading=false,trainAnnouncementPending=null,trainAnnouncementActiveRecording="",trainAnnouncementGeneration=0,trainAnnouncementNextAt=0;
function trainAnnouncementVolume(distance){return clamp(.08+(820-distance)*.00034,.08,.32)}
function stopTrainAnnouncement(){trainAnnouncementGeneration++;trainAnnouncementPending=null;trainAnnouncementLoading=false;if(!trainAnnouncementSource){if(!speechActive)setAudioText();return}const a=ensureAudio(),source=trainAnnouncementSource,gain=trainAnnouncementGain;trainAnnouncementSource=null;trainAnnouncementGain=null;trainAnnouncementActiveRecording="";trainAnnouncementNextAt=Math.max(trainAnnouncementNextAt,performance.now()+3500);if(gain){gain.gain.cancelScheduledValues(a.currentTime);gain.gain.setTargetAtTime(.0001,a.currentTime,.08)}try{source.stop(a.currentTime+.3)}catch{}if(!speechActive)setAudioText()}
function requestTrainAnnouncement(distance,recording="",priority=AUDIO_TEXT_PRIORITY.TRAIN){if(!state.voiceOn||!state.started||state.gameOver)return;const request={distance,recording:recording||TRAIN_ANNOUNCEMENT_AUDIO[trainAnnouncementIndex++%TRAIN_ANNOUNCEMENT_AUDIO.length],priority};if(request.recording===trainAnnouncementActiveRecording)return;if(!trainAnnouncementPending||request.priority>trainAnnouncementPending.priority)trainAnnouncementPending=request;if(!speechActive&&!trainAnnouncementSource&&!trainAnnouncementLoading)playTrainAnnouncement()}
function playTrainAnnouncement(){if(!state.voiceOn||speechActive||trainAnnouncementSource||trainAnnouncementLoading||!trainAnnouncementPending)return;const request=trainAnnouncementPending,generation=++trainAnnouncementGeneration;trainAnnouncementPending=null;trainAnnouncementLoading=true;prepareRecording(request.recording).then(buffer=>{if(generation!==trainAnnouncementGeneration||!state.started||state.gameOver||!state.voiceOn)return;const a=ensureAudio(),source=a.createBufferSource(),gain=a.createGain();source.audioClass=AUDIO_CLASS.TEXT;source.buffer=buffer;gain.gain.value=trainAnnouncementVolume(request.distance);source.connect(gain).connect(a.destination);trainAnnouncementSource=source;trainAnnouncementGain=gain;trainAnnouncementActiveRecording=request.recording;setAudioText("train-announcement",request.priority);source.onended=()=>{if(trainAnnouncementSource!==source)return;trainAnnouncementSource=null;trainAnnouncementGain=null;trainAnnouncementActiveRecording="";trainAnnouncementNextAt=performance.now()+3500;speechPauseTimer=setTimeout(()=>{if(trainAnnouncementPending){playTrainAnnouncement();return}if(speechQueue.length){playQueuedSpeech();return}setAudioText()},SPEECH_GAP_MS)};source.start()}).catch(()=>trainAnnouncementNextAt=performance.now()+10000).finally(()=>{if(generation!==trainAnnouncementGeneration)return;trainAnnouncementLoading=false;if(!trainAnnouncementSource){if(trainAnnouncementPending)playTrainAnnouncement();else if(speechQueue.length)playQueuedSpeech();else if(!speechActive)setAudioText()}})}
function playQueuedSpeech(){if(speechActive||trainAnnouncementSource||trainAnnouncementLoading||trainAnnouncementPending||!speechQueue.length||!state.voiceOn)return;speechActive=true;setAudioText("dialogue",AUDIO_TEXT_PRIORITY.DIALOGUE);const item=speechQueue.shift(),generation=speechGeneration;if(!item.recording){playSyntheticSpeech(item,generation);return}prepareRecording(item.recording).then(buffer=>{if(generation!==speechGeneration)return;if(item.start)item.start();const source=ensureAudio().createBufferSource();recordedSpeechSource=source;source.audioClass=AUDIO_CLASS.TEXT;source.buffer=buffer;source.connect(ensureAudio().destination);source.onended=()=>{if(recordedSpeechSource===source)recordedSpeechSource=null;finishSpeechItem(item.done,generation)};source.start()}).catch(()=>playSyntheticSpeech(item,generation))}
...
function stopSpeech(completeHumorScold=false,preserveTrain=false){speechGeneration++;speechQueue.length=0;speechActive=false;clearTimeout(speechPauseTimer);if("speechSynthesis" in window)speechSynthesis.cancel();stopRecordedSpeech();if(!preserveTrain)stopTrainAnnouncement();else if(trainAnnouncementPending&&!trainAnnouncementSource&&!trainAnnouncementLoading)playTrainAnnouncement();else if(!trainAnnouncementSource&&!trainAnnouncementLoading&&!trainAnnouncementPending)setAudioText();hideWorldBark();cancelHumorScold(completeHumorScold)}
...
function updateTrainAnnouncement(){let nearest=Infinity;for(const train of trains)for(const car of train.cars)nearest=Math.min(nearest,dist(player.x,player.y,car.x,car.y));if(trainAnnouncementSource){if(!state.voiceOn){stopTrainAnnouncement();return}const a=ensureAudio();trainAnnouncementGain.gain.setTargetAtTime(trainAnnouncementVolume(nearest),a.currentTime,.2);return}if(trainAnnouncementPending){if(!speechActive&&!trainAnnouncementLoading)playTrainAnnouncement();return}if(nearest<820&&state.started&&!state.modal&&!state.gameOver&&state.voiceOn&&performance.now()>=trainAnnouncementNextAt)requestTrainAnnouncement(nearest)}
...
function updateBorderTrains(dt){
 ...
 if(playerHolding&&!playerHoldingLast)requestTrainAnnouncement(0,TRAIN_PLAYER_OBSTRUCTION_AUDIO,AUDIO_TEXT_PRIORITY.TRAIN_OBSTRUCTION);playerHoldingLast=playerHolding;
...
let audio=null,musicTimer=null,musicBus=null,soundEffectBus=null,musicOn=true,musicDucked=false,currentTune=-1,forcedTune=-1,introMusicFinished=false;
const INTRO_MUSIC_AUDIO="./assets/intro-song.mp3";
const introMusicAudio=new Audio(INTRO_MUSIC_AUDIO),MUSIC_LEVEL=.46,MUSIC_READOUT_LEVEL=.14,INTRO_MUSIC_LEVEL=.34,INTRO_MUSIC_READOUT_LEVEL=.1,SOUND_EFFECT_LEVEL=1,SOUND_EFFECT_READOUT_LEVEL=.24;introMusicAudio.preload="auto";introMusicAudio.playsInline=true;introMusicAudio.volume=INTRO_MUSIC_LEVEL;introMusicAudio.dataset.audioClass=AUDIO_CLASS.BACKGROUND;
function applyAudioDucking(){const ducked=musicDucked||audioTextActive();introMusicAudio.volume=ducked?INTRO_MUSIC_READOUT_LEVEL:INTRO_MUSIC_LEVEL;if(!audio)return;const t=audio.currentTime;if(musicBus){musicBus.gain.cancelScheduledValues(t);musicBus.gain.setTargetAtTime(ducked?MUSIC_READOUT_LEVEL:MUSIC_LEVEL,t,.045)}if(soundEffectBus){soundEffectBus.gain.cancelScheduledValues(t);soundEffectBus.gain.setTargetAtTime(ducked?SOUND_EFFECT_READOUT_LEVEL:SOUND_EFFECT_LEVEL,t,.045)}}
function setMusicDucked(ducked){musicDucked=ducked;applyAudioDucking()}
function createMusicBus(){const bus=audio.createGain(),filter=audio.createBiquadFilter();bus.audioClass=AUDIO_CLASS.BACKGROUND;bus.gain.value=musicDucked||audioTextActive()?MUSIC_READOUT_LEVEL:MUSIC_LEVEL;filter.type="lowpass";filter.frequency.value=1600;filter.Q.value=.45;bus.connect(filter).connect(audio.destination);return bus}
...
function startMusic(event){
 if(!musicOn)return;if(event){if(!audio)audio=new (window.AudioContext||window.webkitAudioContext)();audio.resume()}if(!introMusicFinished){introMusicAudio.play().catch(()=>{});return}if(musicBus)return;if(!audio)audio=new (window.AudioContext||window.webkitAudioContext)();audio.resume();musicBus=createMusicBus();removeMusicUnlockListeners();scheduleTheme()
}
