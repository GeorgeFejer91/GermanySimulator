...
for(let roadIndex=0;roadIndex<horizontalRoads.length;roadIndex++)for(const dir of [-1,1]){
 ...
 for(let i=0;i<count;i++){const color=trafficPalettes[kind][(i+roadIndex*2+(dir>0?1:0))%trafficPalettes[kind].length],offset=(i+(dir>0?.18:.62)+Math.random()*.34)/count;trafficCars.push({id:`verkehr-${roadIndex}-${dir}-${i}`,lane:`${roadIndex}:${dir}`,kind,color,x:TRAFFIC_MIN_X+offset*TRAFFIC_LANE_LENGTH,y:laneY,dir,angle:dir>0?0:Math.PI,cruiseSpeed:105+Math.random()*55,speed:80+Math.random()*32,acceleration:54+Math.random()*28,blockedByPlayer:false,queued:false,hold:0,honkCooldown:Math.random()*1.4,honkFlash:0})}
}
...
function updateTraffic(dt){
 for(const car of trafficCars){
  ...
  let queueGap=Infinity;for(const other of trafficCars)if(other!==car&&other.lane===car.lane)queueGap=Math.min(queueGap,trafficForwardGap(car,other.x)-TRAFFIC_CAR_RADIUS*2);
  ...
  car.speed+=clamp(target-car.speed,-rate*dt,rate*dt);const intended=Math.max(0,car.speed*dt),advance=Math.min(intended,Math.max(0,obstacleGap-TRAFFIC_STOP_GAP));car.x+=car.dir*advance;if(car.x>TRAFFIC_MAX_X)car.x-=TRAFFIC_LANE_LENGTH;else if(car.x<TRAFFIC_MIN_X)car.x+=TRAFFIC_LANE_LENGTH;
