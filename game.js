...
const TRAIN_CAR_OFFSETS=[780,520,260,0,-260,-520,-780],TRAIN_CAR_HALF_LENGTH=112,TRAIN_CAR_HALF_WIDTH=46,TRAIN_MIN_GAP=1820,TRAIN_PLAYER_STOP_GAP=910,TRAIN_PLAYER_LOOKAHEAD=2100,TRAIN_BOUNCE_PAUSE=.82,TRAIN_AUDIO_RADIUS=820,TRAIN_NEARBY_AUDIO_RADIUS=360;
const trainSeeds=[
...
const AUDIO_CLASS=Object.freeze({TEXT:"audio-text",BACKGROUND:"background-music",EFFECT:"sound-effect"}),STIMULUS_PRIORITY=Object.freeze({AMBIENT:1,REACTIVE:2,FEATURED:3,NEARBY:4,CRITICAL:5}),AUDIO_MIX=Object.freeze({FOREGROUND:1,BACKGROUND:.28,ATTACK_SECONDS:.12,RELEASE_SECONDS:.4,REQUIRED_GAP_MS:250,AMBIENT_GAP_MS:2500,AMBIENT_TTL_MS:4000});
...
let activeAudioText=null,activeStimulus=null,stimulusTimer=null,stimulusGeneration=0,stimulusSequence=0,nextAmbientAt=0,recordedSpeechSource=null,recordedSpeechGain=null,foregroundBus=null,subtitleFrame=0;
...
function answerCitizenshipQuiz(index){
 ...
 if(index===q.answer){uiTone(980,.1,"square",.04);addGermanness(1,"RICHTIG · "+(q.type==="fahrschule"?q.source:"AUFGABE "+q.source))}else{uiTone(170,.16,"sawtooth",.045);addGermanness(-1,"FALSCH · RICHTIG: "+copy.choices[q.answer]);showWorldBark(character?character.name+" · "+character.title:"ENTTÄUSCHTER PRÜFUNGSBEAUFTRAGTER","Nein! Nein! Nein!",true,"./assets/voices/quiz-wrong-answer.mp3","",{family:"quiz-result",priority:STIMULUS_PRIORITY.CRITICAL,ambient:false})}
}
...
let audio=null,musicTimer=null,musicBus=null,soundEffectBus=null,musicOn=true,musicDucked=false,currentTune=-1,forcedTune=-1,introMusicFinished=false,introRampFrame=0,sungRampFrame=0,sungMusicActive=false,eightBitTracksSinceSong=0,wurstSongPending=false,wurstSongPlayed=false;
...
function playWurstSong(){if(!musicOn||!wurstSongPending)return;wurstSongPending=false;wurstSongPlayed=true;clearTimeout(musicTimer);stopIntroMusic();sungMusicAudio.pause();sungMusicActive=false;musicBus?.disconnect();musicBus=null;playSungMusic(WURST_MUSIC)}
...
document.getElementById("voice-toggle").onclick=()=>{state.voiceOn=!state.voiceOn;document.getElementById("voice-toggle").textContent=state.voiceOn?"VOICE ON":"VOICE OFF";if(!state.voiceOn){stopSpeech(true);state.dialogueVoiceToken=(state.dialogueVoiceToken||0)+1;setDialogueVoiceBusy(false)}};
 function keydown(e){if(["ArrowUp","ArrowDown","ArrowLeft","ArrowRight"].includes(e.code))e.preventDefault();keys[e.code]=true;if(e.repeat)return;if(e.code==="KeyS"&&!state.started&&!document.getElementById("intro").classList.contains("hidden")){startGame(true);return}if(e.code==="KeyE")interact();if(e.code==="KeyQ"||e.key==="§")useLawPower()}function keyup(e){keys[e.code]=false}function clearInput(){for(const code in keys)keys[code]=false}addEventListener("keydown",keydown);addEventListener("keyup",keyup);addEventListener("blur",clearInput);document.addEventListener("visibilitychange",()=>{if(document.hidden)clearInput()});
