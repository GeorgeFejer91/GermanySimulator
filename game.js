...
function speakMerkelLine(text,{urgent=false,done}={}){if(text!==MERKEL_AUDIO_LINE){speak(text,{urgent,done});return}if(!state.voiceOn){if(done)done();return}speechQueue.push({text,urgent,done,recorded:true});playQueuedSpeech()}
function stopSpeech(){speechGeneration++;speechQueue.length=0;speechActive=false;clearTimeout(speechPauseTimer);if("speechSynthesis" in window)speechSynthesis.cancel();stopMerkelRecording()}
