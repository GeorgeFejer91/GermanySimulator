...
(async()=>{
  ...
  function makeWirtschaftswunderSite(site){
    ...
    const vertexShader="varying vec2 vUv;void main(){vUv=uv;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.0);}",fragmentShader="uniform float uTime;uniform float uLayer;uniform float uOpacity;varying vec2 vUv;void main(){vec2 p=(vUv-.5)*2.0;float r=length(p);if(r>1.0)discard;float a=atan(p.y,p.x);float spin=uTime*(7.2+uLayer*2.4);float arm=sin(a*7.0-r*34.0+spin);float wake=sin(r*61.0-spin*1.35+a*2.0);float groove=smoothstep(-.45,.82,arm*.78+wake*.22);float rim=smoothstep(.66,.98,r);float shade=(.006+.075*groove)*pow(r,.72);vec3 color=vec3(shade*.42,shade*.47,shade*.54)+rim*.018;color*=smoothstep(.055,.31,r);float alpha=(1.0-smoothstep(.96,1.0,r))*uOpacity;gl_FragColor=vec4(color,alpha);}";
    const layers=[1,.81,.61].map((scale,index)=>{const material=new T.ShaderMaterial({uniforms:{uTime:{value:0},uLayer:{value:index},uOpacity:{value:index? .48:1}},vertexShader,fragmentShader,transparent:index>0,depthWrite:index===0,side:T.DoubleSide}),mesh=new T.Mesh(new T.CircleGeometry(mouth,72),material);mesh.rotation.x=-Math.PI/2;mesh.scale.setScalar(scale);mesh.position.set(centerX,.036+index*.022,centerZ);mesh.renderOrder=3+index;world.add(mesh);return{mesh,material,index}});
    ...
    function siteSign(x,z){const group=new T.Group();for(const px of [-1.35,1.35])box(.09,1.42,.09,M.metal,px,.71,0,group);box(3.25,1.02,.13,yellow,0,1.43,0,group);const material=new T.MeshBasicMaterial({map:signTexture,side:T.DoubleSide}),front=new T.Mesh(new T.PlaneGeometry(3.08,.86),material),back=front.clone();front.position.set(0,1.43,.071);back.position.set(0,1.43,-.071);back.rotation.y=Math.PI;group.add(front,back);group.position.set(x,0,z);world.add(group)}
    const signX=site.siteW*S*.46,signZ=site.siteH*S*.43;siteSign(centerX-signX,centerZ-signZ*.45);siteSign(centerX+signX,centerZ+signZ*.45);siteSign(centerX-signX*.7,centerZ+signZ);siteSign(centerX+signX*.7,centerZ-signZ);
    ...
    return{site,centerX,centerZ,layers,rim}
  }
  const wirtschaftswunderVisual=makeWirtschaftswunderSite(bridge.wirtschaftswunderSite);
  function updateWirtschaftswunder(now){
    if(!wirtschaftswunderVisual)return;const visual=wirtschaftswunderVisual,viewX=Math.max(-.16,Math.min(.16,(camera.position.x-visual.centerX)*.012)),viewZ=Math.max(-.16,Math.min(.16,(camera.position.z-visual.centerZ)*.012));
    for(const layer of visual.layers){layer.material.uniforms.uTime.value=now*.001;const depth=layer.index*.42;layer.mesh.position.x=visual.centerX-viewX*depth;layer.mesh.position.z=visual.centerZ-viewZ*depth;layer.mesh.rotation.z=-now*.0009*(layer.index+1)}visual.rim.rotation.z=now*.0018
  }
  ...
  window.Germany3D={ready:true,isWorldPointVisible,sync(){
    ...
    const traffic=bridge.getTrafficCars?.()||[];traffic.forEach(car=>{let slot=trafficCarMeshes.get(car);if(!slot){slot=makeTrafficCarSlot(car);trafficCarMeshes.set(car,slot)}const sink=car.vortexSink||0,scale=Math.max(.055,1-sink*.93);slot.group.visible=car.vortexPhase!=="swallowed";slot.group.position.set(X(car.x),.07-sink*.72,Z(car.y));slot.group.scale.setScalar(scale);slot.group.rotation.order="YXZ";slot.group.rotation.set(-sink*1.18,Math.PI/2-car.angle,Math.sin((car.vortexSpin||0)*1.7)*sink*.62);slot.tail.color.setHex(car.queued?0xff3026:0x762720)});for(const [car,slot] of trafficCarMeshes)if(!traffic.includes(car)){world.remove(slot.group);trafficCarMeshes.delete(car)}
