...
The rebuilt Reichstag preserves its authored proportions through uniform fitting inside the existing solid parcel, with a stone apron around its setbacks. Its glass material retains authored transparency through building occlusion. Repeated city models load once and clone shared geometry; building fade materials remain per-instance. Roads and pavements stay aligned to simulation rectangles, with small deterministic surface textures and instanced kerbs/markings/drains; Blender is reserved for discrete objects. Four benches, four bins, four bollards and two bicycle racks occupy checked free forecourts using the existing prop collision path. There is no new runtime service, loader dependency, or alternate mobile asset tree.

## 2026-09-25 — Kiesinger historical monument

The southeast Bundestag plaza holds a large statue of Kurt Georg Kiesinger on a code-native Three.js pedestal. At the user's request on 2026-09-26, the figure became a Blender-authored bronze GLB, subsequently refined against front, three-quarter, profile, and full-body archival photographs. Its facial positions use offline photo-landmark fitting with MediaPipe's Apache-2.0 canonical connectivity; the authored skull, ears, hair, hands, suit, and pose complete an artistic reconstruction, not a scan. The editable `.blend`, fitted geometry, license notices, and reference provenance are under `assets/models/kiesinger/`. It uses the existing GLTFLoader and keeps the earlier procedural figure as a missing-model fallback. The user’s height adjustment on 2026-09-26 makes the full statue-and-pedestal assembly match the Reichstag dome’s height through uniform runtime scaling, including either model’s loading fallback; the ground apron and plaque interaction stay in place. The physical plaque and interactive text state his NSDAP membership, work in the Nazi Foreign Office's radio-policy department, 1948 CDU entry, Bundestag and Baden-Württemberg offices, and 1966–1969 chancellorship. A player-triggered German browser-voice summary uses the existing exact-text dialogue broker and English subtitle library. Historical wording is grounded in [Deutsche Biographie](https://www.deutsche-biographie.de/dbo080414.html), [Haus der Geschichte](https://www.hdg.de/lemo/biografie/kurt-georg-kiesinger.html), and [the Bundestag's election record](https://www.bundestag.de/dokumente/textarchiv/1966-12-01-kiesinger-481690). The linked person is Kiesinger; Kurt Eisner is a different historical figure.
...
## 2026-09-24 — Bounded simulation and audio-sequence repair

Keep the static, single-runtime architecture. The stimulus broker now guards completion callbacks that replace its active owner, so a premature HUM-01 reprimand cannot clear the following page reading or welcome line. Wrong quiz answers reserve their `Nein! Nein! Nein!` cue before score commentary. Police and jaywalking barks recheck their triggering state at admission; delayed police pass-bys and fax effects also recheck their source event before playback. Failed recording fetches may retry on a later request.
