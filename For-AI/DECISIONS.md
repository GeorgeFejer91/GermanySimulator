...
The soundtrack starts on the first browser-permitted interaction with the user-supplied `assets/intro-song.mp3`, whose first 24 seconds are removed in the shipped file. That recording plays once only, shares the existing music bus so mute and form-readout ducking still apply, and then hands off to the weighted synthesized German-song catalog. A missing or undecodable intro skips directly to that catalog. The local source and shipped checksums remain recorded in `CREDITS.md`, without an unsupported redistribution-license claim.
