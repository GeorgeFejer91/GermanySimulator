...
- Pressing Start first opens the two-page satirical `HUM-01/DE` humor-competence declaration, presented in Fraktur as an aged printout from an invented humor office. Sheet one remains visibly open until its complete definition text has been read aloud and its acknowledgments, deliberately awkward stamp buttons, and handwritten signature are complete. Sheet two also starts reading automatically, but its reading is not a progression gate: the player may sign, complete, and submit it before the audio finishes. The player draws each of the two signatures directly with a mouse, trackpad, stylus, or touch input; a tap alone does not count, and signature strokes are neither serialized nor persisted. German copy describes concepts in native wording instead of importing labels such as “Deadpan.” Clicking the title screen's original English button is the only gate that selects the form's English text and speech; Deutsch buttons neither select nor clear it. The title-screen joke still turns the clicked English choice into another visible `Deutsch` button, and no UI reveals or persists the preference. Finishing the declaration starts the normal Berlin welcome and gameplay; it is satire, not a real contract or legal test.
