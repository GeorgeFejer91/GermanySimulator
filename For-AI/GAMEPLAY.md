...
- A wrong quiz answer triggers an exact visible-and-spoken `Nein! Nein! Nein!` sting through the existing local browser voice. It is an original runtime performance, not audio extracted from a film, and it continues to respect the player's voice-off setting.
