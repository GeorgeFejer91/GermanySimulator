...
- Keep the user-supplied Bayern and Alice recordings as local foreground MP3s under `assets/voices/bayern/` and `assets/voices/alice-weidel/`, with exact-text runtime mappings and checksums in `assets/voices/LICENSES.md`. Alice's source recordings are user-supplied satirical material, not verified quotations or factual claims. Every atlas-backed character uses its original complete image-generated key sheet under `assets/sprite-sources/` as immutable visual authority. `tools/build-identity-locked-sprite-atlas.py` maps each of the 32 runtime cells to one whole authored key, downsamples only in premultiplied-alpha space, and records exact source/runtime pixel hashes in `identity-audit.json`. It never morphs, cross-fades, optically flows, per-frame recenters, or rebuilds a character from alternate cutout artwork. The shared Three.js staging canvas remains the only runtime path; no skeleton or authoring dependency ships to the browser. Follow `SPRITE-GENERATION-PROTOCOL.md` for the fixed-cell contract, direction mapping, build command, and signed visual acceptance.
- The two accepted ordinary towel pedestrians use the same identity-locked builder, verification ledger, Three.js texture path and 32×3 runtime grid as featured sprites. Their authored key sheets retain the accepted grumpy towel-tourist identities and signature towel props. Direction is selected from the movement vector; only the whole horizontal side row mirrors for left travel. Built-in ImageGen output becomes source authority only after full-resolution identity, anatomy, alpha, prop, and all-direction review. `tools/verify-sprite-animation.py` proves that each final cell is its named authored pose and is mandatory before commit. Pre-rig originals remain under `assets/sprite-archive/pre-rig-20260921/`; the rejected cutout-rig outputs remain under `assets/sprite-archive/pre-identity-lock-20260922/`. Neither archive is a runtime tree.
