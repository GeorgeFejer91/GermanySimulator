...
- The five local train-announcement recordings rotate only while the player is near either loop. They use a dedicated distance-scaled ambient Web Audio source, never open the dialogue textbox, stop outside the rail vicinity or during a modal, and follow the voice toggle. Do not synthesize, infer, or display transcripts for these recordings. EBO obstruction hints remain exact visible-and-spoken gameplay warnings through the existing serialized speech path.
