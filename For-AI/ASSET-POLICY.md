...
- `game.js` owns the two rounded-loop paths, eight train centers, 56 articulated car transforms, speed/pause incidents, same-lane spacing, player obstruction, live queue depth, and announcement timing. Each seven-car array belongs to one logical train. `world3d.js` only renders that state and must not run a second train simulation in the canonical game.
- The WebGL renderer loads the three Kenney car models once and clones them into the eight articulated consists, using one front, five middle, and one rear body plus procedural flexible gangways. Each car follows its shared semantic transform independently through curves. Keep their embedded geometry and shared texture local; there is no runtime asset host.
