...
- `game.js` owns the two rounded-loop paths around the 9,840 × 4,240 world, eight train centers, 56 articulated car transforms, per-train speed and acceleration, unexplained pauses, contact stops and reversals, same-lane spacing, player obstruction, live stopped-train count, and announcement timing. Each seven-car array belongs to one logical train. `world3d.js` only renders that state and must not run a second train simulation in the canonical game.
- The WebGL renderer loads each rolling-stock source once and clones them into the eight articulated consists, using one Kenney front, five full-length Open L-Gauge n-Wagen middle coaches, one Kenney rear, and short procedural flexible gangways. Each car follows its shared semantic transform independently through curves. Keep geometry, the n-Wagen's tiny embedded palette, and all runtime assets local; there is no runtime asset host.
...
- German station announcements and EBO obstruction hints are synthesized locally by the browser from the exact visible string. Rail speech deliberately uses a slow, flat installed German browser voice; no recorded announcement asset or clone of a real DB employee is required. Long announcements must reflect the current train state when possible—unexplained pause, train collision and reversal, stopped traffic, or player obstruction—and community anecdotes must stay visibly labeled unverified and fictionalized.
