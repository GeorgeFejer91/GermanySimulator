...
- The two accepted ordinary towel pedestrians use ImageGen-authored contact sheets only as proposals. `tools/register-sprite-grid.py --component-grid --normalize-scale` isolates complete figures, bounds proposal-scale drift, and assembles each type into an 8×3, 256 px side/down/up key sheet. The shared transition builder derives one 32×3, 128 px full-RGBA atlas (96 runtime frames) per type. Direction is selected from the movement vector; only the horizontal side row may mirror for left travel. Their source and runtime files stay local at `assets/sprite-sources/crowd-towel-*-keys.png` and `assets/crowd-towel-*.png`; no raw ImageGen proposal is shipped. Ordinary crowd sprites share the same Three.js staging-canvas and texture path as featured sprites, with a smaller uniform world scale and no separate loader or animation framework. `tools/verify-sprite-animation.py` and the SHA-bound visual ledger are mandatory before commit; any pixel edit invalidates sign-off.
