...
- Keep the user-supplied Bayern and Alice recordings as local foreground MP3s under `assets/voices/bayern/` and `assets/voices/alice-weidel/`, with exact-text runtime mappings and checksums in `assets/voices/LICENSES.md`. Alice's source recordings are user-supplied satirical material, not verified quotations or factual claims. Every atlas-backed character now has one high-resolution side/front/back cutout-parts authority under `assets/sprite-sources/rigs/`. `tools/build-rigged-sprite-atlas.py` renders eight review keys and all 32 direct joint-driven frames per row using fixed head/torso registration, two-bone leg IK, stance-foot pinning, alternating swing lift, signed front/back depth, limb overlap and bounded joint stitching. It emits 256 px review keys, 128 px full-RGBA runtime cells with zero RGB behind alpha zero, and a hash-bound one-to-one pose audit for final-atlas direction checks. Silhouette-based per-frame recentering and complete-character optical-flow interpolation are not production authorities. The shared Three.js staging canvas remains the only runtime path; no skeleton or authoring dependency ships to the browser. Follow `SPRITE-GENERATION-PROTOCOL.md` for the exact three-view/fourteen-part contract, gait phases, build command and visual acceptance.
- The two accepted ordinary towel pedestrians use the same rig renderer, verification ledger, Three.js texture path and 32×3 runtime grid as featured sprites. Their source parts retain the accepted grumpy towel-tourist identities and signature towel props. Direction is selected from the movement vector; only the horizontal side row mirrors for left travel. Built-in ImageGen output is source material only after full-resolution identity, anatomy, alpha and prop review; `assets/sprite-sources/rigs/PROVENANCE.md` records the accepted prompt contract. `tools/verify-sprite-animation.py` binds parts/key/runtime hashes and is mandatory before commit. Replaced sheets and atlases remain recoverable only under `assets/sprite-archive/pre-rig-20260921/`, never as a second runtime tree.
