...
- Keep the 13 generated §-power readings under `assets/voices/laws/`, indexed directly to the fixed law deck. They must use the exact displayed text, retain the German browser voice as a missing-file fallback, and record their CC0 references, local XTTS-v2 generation method, durations, and checksums in `assets/voices/LICENSES.md`. The cloning model is an offline production tool, never a runtime dependency or shipped model.
