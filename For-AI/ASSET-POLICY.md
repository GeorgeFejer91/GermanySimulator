...
- The five local train-announcement recordings rotate only while the player is near either loop. They are priority-managed `audio-text` events: one recording plays to completion, later dialogue waits, and background music plus the shared sound-effect bus duck for intelligibility. A new player-caused train stop always reserves the Buxtehude file ahead of ordinary proximity cues. Train recordings never open the dialogue textbox and still follow the voice toggle. Do not synthesize, infer, or display transcripts for these recordings. EBO obstruction hints remain exact visible-and-spoken gameplay warnings through the same serialized audio-text path.
